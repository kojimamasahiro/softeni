...
const detailContainsEntryNos = (detail, entryNos) => {
  ...
  const known = new Set(entries.map((entry) => parseEntryNo(entry?.entryNo)).filter((value) => value !== null));
...
export const resolveMatchSiteLink = (match, { detailsRoot }) => {
  ...
  let candidateFiles;
  try {
    candidateFiles = fs.readdirSync(yearDir).filter((fileName) => fileName.endsWith('.json'));
  } catch {
    return null;
  }
