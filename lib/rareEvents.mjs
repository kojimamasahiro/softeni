// lib/rareEvents.mjs
// 希少ポイント/ゲームイベント検知（大会単位の相対評価・ポジティブ限定）。
...
// 方針（2026-07-11 決定）:
//  - 絶対閾値でなく「その大会（＝当サイトでスコア記録した試合の範囲）で最大」の相対評価を既定にする。
...
export const RARE_EVENT_CONFIG = {
  scope: 'tournament', // 将来 'season' | 'all-time' へ拡張（P3）
  categories: {
...
export const RARE_EVENT_SCOPE_NOTE = '「この大会」は当サイトでスコア記録した試合の範囲での比較';
...
const baseEvent = (kind, edition, match) => ({
  kind,
  scope: 'tournament',
...
/**
 * 1開催ぶんの試合列から希少イベントを検知する。
 * 戻り値: { events, stats }
 *  - events: RareEvent[]（label 付き・重要度順ではなく KIND_ORDER 順）
 *  - stats: レポート（ハーネス）用の分布データ。閾値調整の材料。
 */
export const detectRareEvents = (edition, config = RARE_EVENT_CONFIG) => {
  const events = [];
  const stats = {
    matchCount: edition.matches.length,
    pointCount: 0,
  ...
  if (cats['longest-rally']?.enabled && bestRally.rally >= (cats['longest-rally'].minRally ?? 1)) {
    for (const { match, game, point } of applyMaxTies('longest-rally', bestRally.entries)) {
      events.push({
        ...baseEvent('longest-rally', edition, match),
        gameId: game.id,
        ...
        label: `この大会最長の${bestRally.rally}本ラリー`,
      });
    }
  }
  ...
  if (cats['service-ace']?.enabled) {
    ...
    for (const { match, game, point } of applyMaxTies('service-ace', aces)) {
      events.push({
        ...baseEvent('service-ace', edition, match),
        gameId: game.id,
        gameNumber: game.game_number,
        pointId: point.id,
        pointNumber: point.point_number,
        ...
        videoUrl: buildVideoUrl(match, point),
        label: soleAce ? 'この大会唯一のサービスエース' : `サービスエース（この大会で計${aces.length}本）`,
      });
    }
  }
  ...
  if (cats['longest-deuce']?.enabled && bestDeuce.totalPoints >= (cats['longest-deuce'].minTotalPoints ?? 1)) {
    for (const { match, game, lastPoint } of applyMaxTies('longest-deuce', bestDeuce.entries)) {
      events.push({
        ...baseEvent('longest-deuce', edition, match),
        gameId: game.id,
        gameNumber: game.game_number,
        ...
        label: `この大会最長のデュースゲーム（第${game.game_number}ゲーム・全${bestDeuce.totalPoints}ポイント）`,
      });
    }
  }
  ...
  if (cats['biggest-comeback']?.enabled && bestComeback.deficit >= (cats['biggest-comeback'].minDeficit ?? 1)) {
    for (const { match, game, team, lastPoint } of applyMaxTies('biggest-comeback', bestComeback.entries)) {
      events.push({
        ...baseEvent('biggest-comeback', edition, match),
        gameId: game.id,
        gameNumber: game.game_number,
        pointId: lastPoint.id,
        pointNumber: lastPoint.point_number,
        ...
        videoUrl: buildVideoUrl(match, lastPoint),
        label: `この大会最大の${bestComeback.deficit}点差逆転ゲーム（第${game.game_number}ゲーム）`,
      });
    }
  }
  ...
  if (cats['longest-point-streak']?.enabled && bestStreak.streak >= (cats['longest-point-streak'].minStreak ?? 1)) {
    for (const { match, team, start, endGame } of applyMaxTies('longest-point-streak', bestStreak.entries)) {
      if (!start) continue;
      events.push({
        ...baseEvent('longest-point-streak', edition, match),
        ...
        label: `この大会最多の${bestStreak.streak}連続ポイント`,
      });
    }
  }
  ...
  for (const pattern of config.patterns ?? []) {
    ...
    for (const { match, game, point } of hits) {
      events.push({
        ...baseEvent('pattern', edition, match),
        ...
        videoUrl: buildVideoUrl(match, point),
        label: hits.length === 1 ? `この大会唯一の${pattern.label}` : `${pattern.label}（この大会で計${hits.length}本）`,
      });
    }
  }

  for (const event of events) {
    event.scopeNote = RARE_EVENT_SCOPE_NOTE;
...
/**
 * 全試合から全開催ぶんのイベントを検知する。
 * 戻り値: { events, editions: [{ key, tournamentId, year, stats }] }
 */
export const detectAllRareEvents = (matches, config = RARE_EVENT_CONFIG) => {
  const events = [];
  const editions = [];
  for (const edition of groupMatchesByEdition(matches).values()) {
    const { events: editionEvents, stats } = detectRareEvents(edition, config);
    events.push(...editionEvents);
    editions.push({ key: edition.key, tournamentId: edition.tournamentId, year: edition.year, stats });
  }
