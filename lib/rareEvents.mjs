...
export const RARE_EVENT_CONFIG = {
  ...
  categories: {
    ...
    // 相手のマッチポイントをしのいで勝った試合（しのいだ本数が最多のもの）。
    // ゲームカウントで勝者が決まった試合だけが対象（status は見ない。古い記録は決着済みでも draft のまま）
    'match-points-saved': { enabled: true, minSaved: 1, maxTies: 3 },
  },
  /**
...
const KIND_ORDER = ['longest-rally', 'service-ace', 'longest-deuce', 'biggest-comeback', 'longest-point-streak', 'match-points-saved', 'pattern'];
...
/** ゲームカウントで決着していれば勝者（'A' | 'B'）、未決着なら null。 */
const getMatchWinnerByGames = (match) => {
  const requiredWins = getRequiredWins(match.best_of ?? 0);
  const games = match.games ?? [];
  if (games.filter((game) => game.winner_team === 'A').length >= requiredWins) return 'A';
  if (games.filter((game) => game.winner_team === 'B').length >= requiredWins) return 'B';
  return null;
};

/**
 * 試合の勝者がしのいだ相手のマッチポイント（相手があと1ポイントで試合に勝つ局面で、勝者が取ったポイント）を列挙する。
 * 未決着の試合は空配列。
 */
const listSavedMatchPoints = (match) => {
  const matchWinner = getMatchWinnerByGames(match);
  if (!matchWinner) return [];
  const opponent = matchWinner === 'A' ? 'B' : 'A';
  const requiredWins = getRequiredWins(match.best_of ?? 0);
  const saved = [];
  let opponentGames = 0;
  ...
  for (const { game, winPoints } of listGamesWithRules(match)) {
    ...
    for (const point of sortedPoints(game)) {
      if (point.winner_team !== 'A' && point.winner_team !== 'B') continue;
      const isMatchPoint = opponentGames === requiredWins - 1 && opponentScore + 1 >= winPoints && opponentScore + 1 - winnerScore >= 2;
      if (point.winner_team === matchWinner) {
        if (isMatchPoint) saved.push({ game, point });
        winnerScore += 1;
      } else {
        opponentScore += 1;
      }
    }
    if (game.winner_team === opponent) opponentGames += 1;
  }
  return saved;
};
...
export const detectRareEvents = (pool, config = RARE_EVENT_CONFIG) => {
  ...
  const stats = {
    ...
    matchPointsSaved: [], // { saved, matchId }
  };
  ...
  // --- 試合横断走査（match-points-saved） ---
  let bestMatchPointsSaved = { saved: 0, entries: [] };

  for (const match of pool.matches) {
    const saved = listSavedMatchPoints(match);
    if (saved.length === 0) continue;
    stats.matchPointsSaved.push({ saved: saved.length, matchId: match.id });
    if (saved.length > bestMatchPointsSaved.saved) {
      bestMatchPointsSaved = { saved: saved.length, entries: [{ match, saved }] };
    } else if (saved.length === bestMatchPointsSaved.saved) {
      bestMatchPointsSaved.entries.push({ match, saved });
    }
  }
  ...
  if (cats['match-points-saved']?.enabled && bestMatchPointsSaved.saved >= (cats['match-points-saved'].minSaved ?? 1)) {
    for (const { match, saved } of applyMaxTies('match-points-saved', bestMatchPointsSaved.entries)) {
      // 飛び先は最初にしのいだマッチポイント（そこから動画を見ると逆転の流れが追える）
      const { game, point } = saved[0];
      events.push({
        ...baseEvent('match-points-saved', scope, match),
        gameId: game.id,
        gameNumber: game.game_number,
        pointId: point.id,
        pointNumber: point.point_number,
        subjectTeam: getMatchWinnerByGames(match),
        detail: { saved: bestMatchPointsSaved.saved, lastGameNumber: saved[saved.length - 1].game.game_number },
        videoUrl: buildVideoUrl(match, point),
        label: `${wording.sup}最多、マッチポイントを${bestMatchPointsSaved.saved}本しのいで勝利`,
      });
    }
  }
