...
const RALLY_BUCKET_ORDER: RallyBucket[] = ['1-2', '3-4', '5-8', '9+', 'unknown'];
...
const getRateReliability = (metric: RateMetric, minSample: number = ANALYSIS_THRESHOLDS.MIN_RATE_SAMPLE): AnalysisReliability => {
  if (metric.denominator === 0) return 'none';
  if (metric.denominator < minSample) return 'low';
  return 'ok';
};

const getPresenceReliability = (count: number): AnalysisReliability => (count > 0 ? 'ok' : 'none');

const isDeuceScore = (scoreA: number, scoreB: number, pointsToWin: number) => scoreA === scoreB && scoreA >= pointsToWin - 1;

const getRallyBucket = (point: Pick<Point, 'rally_count' | 'double_fault'>): RallyBucket => {
...
const buildMomentumSegments = (points: ReconstructedPointContext[]): MomentumSegment[] => {
...
const buildTeamInsights = (team: TeamKey, metrics: TeamAnalysisMetrics): TeamPerspectiveInsights => {
...
const getHintTarget = (match: Match, team: TeamKey, playerName?: string): ImprovementHint['target'] => {
...
const getConfidence = (sampleSize: number, minSample: number, isStrongSignal: boolean): ImprovementHintConfidence => {
  if (sampleSize < minSample) return 'low';
  if (isStrongSignal && sampleSize >= minSample * 2) return 'high';
  return 'medium';
};

const getConfidenceReason = (confidence: ImprovementHintConfidence, sampleSize: number, minSample: number) => {
  if (confidence === 'low') {
...
const getMetricDropFromOverall = (metric: RateMetric, overallMetric: RateMetric) => {
...
const getOpponentGamePointSaveRate = (team: TeamKey, reconstructedPoints: ReconstructedPointContext[]) => {
...
const getDominantLoserPlayer = (match: Match, team: TeamKey, reconstructedPoints: ReconstructedPointContext[]) => {
...
const buildMetric = (key: ImprovementHintSourceMetricKey, label: string, metric: RateMetric): ImprovementHintSourceMetric => ({
  key,
  label,
...
const toReviewPoint = (context: ReconstructedPointContext): ImprovementHintReviewPoint => ({
...
const getPointAt = (reconstructedPoints: ReconstructedPointContext[], gameNumber: number, pointNumber: number) =>
  reconstructedPoints.find((context) => context.gameNumber === gameNumber && context.pointNumber === pointNumber) ?? null;

const getPointAfterSegment = (reconstructedPoints: ReconstructedPointContext[], segment: MomentumSegment | null) => {
...
const getPointsBeforeContexts = (reconstructedPoints: ReconstructedPointContext[], points: ReconstructedPointContext[]) =>
  points
    .map((point) => {
...
const getPointsAfterContexts = (reconstructedPoints: ReconstructedPointContext[], points: ReconstructedPointContext[]) =>
  points
    .map((point) => {
      const index = reconstructedPoints.findIndex((context) => context.point.id === point.point.id);
