...
export const getTournamentSortPriority = (tournamentId: string): number =>
  HIGHSCHOOL_TOURNAMENT_PRIORITY[tournamentId] ?? 99;
...
/** ベスト8以上として扱う成績 */
export const HIGH_RESULT_SET = new Set([
  '優勝',
  '準優勝',
  'ベスト4',
  'ベスト8',
...
export const isBest8Result = (result: string): boolean =>
  HIGH_RESULT_SET.has(result);

/** mixed の成績は boys / girls の両ページに表示する */
export const isVisibleGender = (
  entryGender: string,
  pageGender: HighschoolGender,
): boolean => entryGender === pageGender || entryGender === 'mixed';

export const getGenderLabel = (gender: HighschoolGender): string =>
  gender === 'boys' ? '男子' : '女子';
