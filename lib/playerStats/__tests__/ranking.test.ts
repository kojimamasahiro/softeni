...
test('順位表は男女別: gender ごとに別シーズンに分かれる', () => {
  const entries = [
    entry({ tournamentId: 'a', gender: 'boys', categoryId: 'doubles-none-boys', placement: { kind: 'winner' }, isMajorTitle: true }), // 100
    entry({ tournamentId: 'b', gender: 'girls', categoryId: 'doubles-none-girls', placement: { kind: 'winner' }, isNational: true, isMajorTitle: false }), // 60
  ];
  const sp = computeSeasonPoints(entries, DEFAULT_CONFIG).sort((a, b) => a.gender.localeCompare(b.gender));
  assert.strictEqual(sp.length, 2);
  assert.deepStrictEqual(
    sp.map((s) => [s.gender, s.points]),
    [
      ['boys', 100],
      ['girls', 60],
    ],
