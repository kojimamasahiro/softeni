...
- **docs の点検で残った4件**（2026-09-19 の棚卸し。[raw](../raw/2026-09-19-docs-layer-cleanup.md)）:
  - **`適用範囲` の行が 13/41 ページにしかない**（AGENTS.md は全ページに要求）。残りは index.md の仮タグのまま。
    ページを触るときに付けるか、一度に埋めるか。
  - **index.md 以外から参照されていない wiki が3ページ**（`android.md` / `architecture.md` / `project-overview.md`）。
    クロスリンクの規約は新規ページにしか適用されていない。
  - **Compile Log が34本で欠けている**（176本中142本は記入済み）。うち10本は wiki のアーカイブで、
    **compile の「先」なので本来不要**だが例外が明文化されていない。残りは作業リスト型。
  - **ADR の Status が自由文**（23本中、素の `Accepted` は5本。`Accepted。決定日: …` 等）。
    書式を決めれば「Draft のまま放置」を機械で数えられる。
- docs の lint（孤立・Compile Log 欠落・ADR Status 記入漏れ）を CI 化するか。
  **2026-09-19 に `npm run check:wiki` が CI に入った**（リンク切れはゲート、文字数は報告のみ）。残りはこれを育てるかの判断。
...
- **団体戦の対戦ごとの記録（オーダー）の残り**（[ADR-020](../adr/ADR-020-team-match-rubber-details.md)）:
  インターハイの他年度（2022〜2026 は男女とも投入済み。2021年以前は様式未確認。2020 は中止）/
  **ゲームの成立検査（4点先取・デュースは2点差・3-3 の第7ゲームは7点先取）を
  ...
  で落ちるので許可リストが要る。今は取り込みのたびに手で確かめている/
...
- **アジア競技大会2026 の取り込み**（[upcoming-tournaments-runbook.md](./upcoming-tournaments-runbook.md) S11）:
  範囲は 2026-09-19 に決着（**5種目すべて全組・全試合**）。方式A は使っていない。
  残る論点は「**結果が入った後の外国選手の扱い**」——個人種目の外国選手が勝敗を持つので、
  選手ページ・名寄せ・`data/players/index.json` の扱いを会期後に見直す。
- **混合ダブルスの「各組上位2組が通過」は推定**（[ADR-021](../adr/ADR-021-category-competition-format.md)）。
  公式に明記が無く、決勝Tの枠数12と組数6からの当サイトの読み。9/21 の1回戦の組み合わせで確かめ、
  外れていれば `format.summary` を直す。**どの組が1回戦免除になるかも公表されていない**。
