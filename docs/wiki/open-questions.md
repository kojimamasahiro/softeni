...
- docs の検査は `npm run check:wiki` が見る（CI ではリンク切れがゲート、それ以外は報告のみ）。
  **2026-09-19 に、適用範囲の行・wiki の孤立・ADR の Status 書式・Compile Log の欠落も機械チェックに入れ、
  当時の未対応はすべて解消した**（[raw](../raw/2026-09-19-docs-layer-cleanup.md)。直前の突き合わせは
  [2026-09-17 drift レビュー](../raw/2026-09-17-docs-drift-review.md)）。
...
- **アジア競技大会2026 の取り込み**（[upcoming-tournaments-runbook.md](./upcoming-tournaments-runbook.md) S11）:
  ...
  **9/21 の混合ダブルスで実際に勝敗を持ち始めた**（日本選手の通算成績にもアジア大会の試合が入る）。
