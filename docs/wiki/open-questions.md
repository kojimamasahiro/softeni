...
| アイデア | 状況・目的（1行） | 詳細 |
|---|---|---|
| AIが自律的にサービスを改善する仕組み（検知・検証の全体委譲） | **2026-09-06: 発散フェーズだが方向は決着**。「AIソロプレナー」的な関心が出発点で、任せたいのは**課題発見と "床" の底上げ、方向の判断は人間**。姉妹2本（共同探索=人間主導層 / ローカルLLM代替=タスク遂行層）の上位にあたる「AI自身が何をやるか決める」層で、バックログでは初。**委譲の単位は「機能の一部」でなく「1ドメイン丸ごと」**——チーム名寄せ（候補生成→自動OK判定→人手レビューHTML→反映）が**既に完成例**で、②に「自動でよい範囲」がコードで定義されている（`signal:"players"` は常に人手）ため「人間が全部を見なくても最終判断は人間」が成立している。逆に**部分適用は実際に止まった実証がある**——クローラーの `detected-documents.json` が**583件すべて status:"new" のまま3ヶ月放置**、accepted 0件のため反映側スクリプトは一度も動いておらず、クローラー自体も2026-06-12以降停止（**583件は捨てる判断**）。**目的関数にPV・収益は置かない**——PVが上がる選択肢を人間が退けた実例が4件あり（空ページへの広告を自ら417枚除外／優勝予想は実測で当たると分かっても見送り／ADR-010でページを減らす／姓頭文字分割ページ不採用）、PV最適化エージェントなら4件とも逆に判断する。**②（境界の決定）の自動化も要求されており、形式的には成立する**——「提案はLLM・採否は機械」の型が`tuning.repair` / ADR-012 / `ranking:backtest` と**3回独立に発明済み**で、②自体も`GOOD_ENOUGH = 0.75` として既に1箇所で自動化されている。ただし**その境界は一度壊れている**（上限0.74で目安0.75に到達不能・症状は「なんとなく上手くいかない」だけ）ため境界を測る仕組みが要る。学習材料の実測では**人の判断は6件しか残っていない**（レビューHTMLが`localStorage`保存のため）一方、**二経路照合の正解データは潤沢**（49件全項目一致／796名99.6%／20,571試合・不一致0）なので、「人の判断を学ぶ」でなく「独立した第二経路で自分を採点する」経路を採る。設計の骨子は**人間の判断をキュー処理でなく機械の抜き取り検査に使う**こと（人の時間が件数に比例しなくなる）。`check-*`/`verify-*` **11本すべてが事後に生まれており**8月だけで5本＝発見負荷は増加中。**出発点はPDF取り込み系**（境界・自己修復・正解データ・破綻実例が揃っているため）。**2026-09-06: (1)走る場所に着手し、前提の誤りを1つ訂正**——「CIがゼロ」は誤りで、Cloudflare Pages が push ごとに `prebuild` を自動実行していた。真の問題は**prebuild にはゲートしか置けず、合否の付かない「判断待ち一覧」型の検出器は置き場が無くて休眠していた**こと（検出器11本中7本＋テスト系10本が自動実行されていなかった）。休眠中の7本をその場で回したところ **`verify-bracket-layout` が250件の不一致で落ち、原因は2日前(2026-09-04)の取り込み**と判明——検出器は正しく動くのに走らせる契機だけが無かった、という本アイデアの直接の実証になった（データ側の誤りか検証ロジックの未対応かは未判定）。`.github/workflows/checks.yml` を追加し、CFに置けないもの（cron・ゲートと報告の分離・サマリ）を引き受ける形にした。ゲート7本はローカル実測で合計3秒・全PASS。`verify-bracket-layout` は赤いCIを常態化させないため報告側に置いた。**同日中に250件は解消され（PR #86・本戦前の「予選」形式への対応）、報告→ゲートへ昇格済み**＝非対称ルール「締める方向は自由」が初適用され、1周回った。**残る未解決は緩める方向の非対称ルールの検証・抜き取り標本設計・リポジトリへ書き戻せる実行環境（現在 `contents: read`。**先例あり**——2025-05-02に毎日cronでsitemapを生成しbotがpushするworkflowが作られたが2日で無効化され自動コミットは1件も残っていない。廃れた理由は「生成物をコミットして戻す」方式が「ビルド時に生成する」（postbuildのnext-sitemap）に置き換わったためで、**教訓は「書き戻すな」ではなく「生成物を書き戻すな」**。抜き取り監査が要求するのは再生成できない記録＝人の判断・誤り率の履歴なので、`contents: write` を入れるなら対象をそこに限る線引きが要る）・Pythonの依存が未固定でCIに載せられないこと**。**2026-09-06: 「レビュー判断がlocalStorageから出ない」は解決**——判断台帳 `data/teams/review-decisions.json` を追加し、クラスタを配列インデックスでなく**メンバーのteam id**で識別するようにした（旧実装は再生成のたびに判断を取り違えるか捨てるかで、実際にlocalStorageのKEYがv9まで上がり、既存サーバにも「レビュー中は候補を作り直さない（番号がずれるため）」という回避注記があった）。「統合しない」という**否定の判断**と、機械の提案(`proposedAutoOK`)・決定者(`decidedBy`)も残すので、**「自動OKにしたが人が覆した」件数＝自動判定の誤り率**が数えられる＝抜き取り監査の素ができた。調査中に**誤った断定を1つ訂正**——「/apply を受けるサーバは存在しない」としたが`scripts/team-review-server.mjs` は実在した（zshで `tools/*.mjs` がマッチせずgrep自体が中止していた）。新規サーバは作らず既存を拡張した。**2026-09-06: 「リポジトリへ書き戻せる実行環境」も解決**——`review-snapshot.yml` を`checks.yml` とは別ファイルで追加し、`contents: write` はそちらだけに与えた（ゲート用のワークフローに push 権限を持たせないため）。`data/teams/review-history.json` に候補件数と誤り率の推移を残し、**数字が動いた週だけコミットする**（動かない日を書き足すとコミットと本番ビルドだけが増えるため）。着手時に自分の線引きへの訂正が1つ出た——`summarize()` の出力は台帳から再計算できるので厳密には「再生成できない記録」ではない。**実際に復元できないのは merge-candidates の件数と上書きされた判断の上書き前の内容**なので、そこを含める設計にした。**これで抜き取り監査の前提（判断の置き場・書き戻し・誤り率の式と時系列）はすべて揃い、残るのは母数＝人が実際にレビューを何件か通すことだけ**（562クラスタ・うち選手共有104は必ず人手）。未確認は `[skip ci]` が Cloudflare Pages に効くか。**（同日夜のうちに解消・2026-09-11に書き戻し）母数の獲得は完了**——候補358件（選手共有19件を含む）は全件判断済み。**2026-09-11: 監査の2欠陥のうち1つを実装で解決、もう1つは誤診断と判明**——「機械の統合判断が人の目に届く前に適用されて消えるため誤統合を原理的に測れない」は、抜き取り標本の母集団を「台帳に記録済み（＝適用済みで手遅れ）」から「これから自動適用されようとしていて未記録」に変え、`apply-auto-merges.mjs`（判断台帳を一切見ていなかった別経路）にも保留と記録を実装して解決（scratchpadの合成データで一連の流れを検証済み・本番データでの初回実行はまだ）。ついでに「台帳の人の判断を機械が無条件上書きする」別バグも発見・修正。一方「『機械の判定に同意した』を記録する操作が無い」は**誤診断だった**——「確認済にする」ボタンは判断台帳導入時から一貫して`decidedBy:"human"`を正しく記録していた。**2026-09-11: 本番データでも意図的テストを実施し動作確認**（実在する1件の判断を一時的に取り除いて未判断化→draw→保留→apply-auto-mergesが保留を尊重→復元、まで確認し完全にgit checkoutで元に戻した）。**副産物として過去の監査対象141件中58件が「検証されないまま既に統合済み」だったと発覚**——欠陥1が本番データへ与えていた実害の実例（44件は元ペア同士が統合済み・14件は無関係な理由で候補から消えただけ）。明確な誤りの証拠は無いが遡って検証する手段も無いため、対応はせず記録のみ。通常の（テスト目的でない）初回`--draw`はまだ未実行。**2026-09-12: 本アイデアの出発点だったクローラーの583件を削除**（9/6の「捨てる」判断が一度も実行されていなかった＝決定側の書き戻し漏れ）。実測すると放置の原因は9/6の診断（人手が律速）だけでなく、**反映先が決まる候補が20件しか無く残り563件は仕分けても入れる先が無い**＝入口が緩く出口が狭い状態だった。教訓は「検知から作らず出口から作る」。[ADR-001](../adr/ADR-001-local-source-detection-store.md) は Deprecated、[tournaments-local.md](./tournaments-local.md) の該当フローも Deprecated 化。**同日: 抜き取り監査をチーム名寄せから外し PDF 取り込み系へ移す決定**（新規の自動OKクラスタが1件で母集団が成立しない／全件レビューへ縮小し自動統合を廃止／欠陥4をサーバ側の不変条件で修正）。**人の点検で誤統合 11/20（55.0%）と判明し、決定を事後に裏付けた**。原因は名前より出場大会ジャンルを優先していたことで、優先順位を逆転し人の判断を回帰テスト化（CIゲート）。**18クラスタを git 履歴から復元**（37名/22ファイル）。PDF 取り込み系では**抽出器 97.8%→100%**（右段3文字姓のバグ修正）と、**公開データ側の誤り15件の修正**を実施 | [アイデア](../raw/2026-09-06-idea-autonomous-improvement-agent.md) |
