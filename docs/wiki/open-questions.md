...
| wiki の圧縮（読み込みコストの削減） | **一部実施**（2026-09-18〜）。wiki を現在の仕様だけにして毎回の LLM コストを下げる。残りは予算超過ページ（手順は [slim-wiki-page.md](../prompts/slim-wiki-page.md)） | [作業ノート](../raw/2026-09-18-wiki-slimming.md) |
...
- **団体戦の対戦ごとの記録（オーダー）の残り**（[ADR-020](../adr/ADR-020-team-match-rubber-details.md)）:
  インターハイ男子団体（記録PDF未入手）と他年度 / **ゲームごとのポイント（`games`）の見せ方** /
...
- **アジア競技大会2026 の取り込み**（[upcoming-tournaments-runbook.md](./upcoming-tournaments-runbook.md) S11）:
  日本以外の成績をどこまで入れるか（日本の組のエントリーには入る想定・**Assumption**）/
  決勝トーナメントの持ち方（`knockoutDraw` が未公開で、日本の試合だけだと復元が部分的になる）/
  方式B（全試合・外国選手も選手として持つ）をやるかは会期後に判断。
