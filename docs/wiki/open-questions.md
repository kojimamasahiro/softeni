...
- **団体戦の対戦で「不戦勝」を表せない**（ADR-020）。片側がペアを出さない（公式表示 `BYE`）と、
  団体の本数には1勝として入るのに対戦としては書けず、「勝った対戦の数＝親の本数」が壊れる。
  アジア大会2026 男子B組の2試合は、そのためオーダーを持っていない（本数だけ）。
  案: `status` に `walkover` を足し本数は `null` にする。**途中棄権（`retired`）は 2026-09-18 に対応済み**。
