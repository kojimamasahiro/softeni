...
ステータス: **P0 実装済み（2026-07-11）**。score 機能で記録した試合のポイント列から、
大会単位で希少なプレー（名場面）を検知し、試合詳細ページに表示＋SNS 手動投稿用テンプレを生成する。
...
- **表示先は試合詳細ページ**（スコア記録と一緒に「この大会の名場面」ブロックで表示）。
...
- データが少ないうちの誤検知（見かけ上の「大会最長」等）は**許容**。ただし表示・投稿には必ず
  scopeNote（「この大会」＝当サイトでスコア記録した試合の範囲での比較）を添える。

## 設計原則（プランから継承）

- **大会単位の相対評価**を既定にする（絶対閾値は投稿過多 or ゼロ件に振れやすい。「この大会最長」は必ず成立）。
...
public/data/beta-matches/matches/*.json（既存生成物）
  ...
  ↓ 試合詳細ページ「この大会の名場面」ブロック（ポイントへジャンプ＋動画再生）
```

- 大会（開催）単位＝ `tournament_id × tournament_year`。大会に紐づかない試合（野良・練習試合）は対象外。
...
- `src/pages/beta/matches-results/[matchId]/index.tsx` — 「この大会の名場面」ブロック

## 参考実績（2026-07-11、16試合・5開催）

24イベント（うち動画あり3件）。全開催で1件以上成立し「大会単位の相対評価なら歩留まりが安定する」
仮説どおり。zennihon-singles 2026 では 2点差逆転が6ゲームで同値タイ→ maxTies で抑制が機能。
