...
| match-points-saved | 試合の勝者がしのいだ相手のマッチポイントの本数（試合単位）。飛び先は最初にしのいだマッチポイント | minSaved=1, maxTies=3 |

試合単位のカテゴリ（match-points-saved）は、**ゲームカウントで勝者が決まった試合**だけを対象にする。
`status` は見ない（完了の自動記録が入る前に記録した試合は、決着済みでも `draft` のまま残っている）。
成長分析の `isCompletedMatch` と同じ考え方。
