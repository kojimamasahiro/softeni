...
| アイデア | 状況・目的（1行） | 詳細 |
|---|---|---|
| 大会メタデータ基盤（会場・施設・日程・大会要項） | **一部実装済み**（2026-08-25〜）。会場・日程・要項をエンティティ化する。「これから開催」とアジア競技大会（日本代表名簿・種目別日程）まで実装。残りは[実行ランブック](./upcoming-tournaments-runbook.md) | [アイデア](../raw/2026-07-26-idea-tournament-metadata-platform.md) |
