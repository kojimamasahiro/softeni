...
### 大会ハブの歴代優勝者表・行見出し（2026-08-26 修正）

行見出しは原則**性別のみ**（男子 / 女子 / 混合）にする。グループ見出し（ダブルス / 団体戦 /
シングルス）が種目を示しているので、行ごとに「男子ダブルス」と繰り返さないため。

ただし**同じグループ見出しの中で短縮ラベルが衝突する大会はフルの種目名を使う**。
年齢区分のある大会がこれに当たる。

| 大会 | 行見出し |
|---|---|
| 天皇賜杯・インターハイ 等 | 男子 / 女子（短縮） |
| 全日本社会人 | 一般男子 / 一般女子 / 男子35歳 / 女子35歳 / 男子45歳 / 女子45歳 |
| 全日本シニア | 男子50歳 / 女子50歳 / …（年齢別） |
| 全日本ミックス | 一般 / 35歳 / 45歳 / …（全種目が `mixed` なので短縮すると全部「混合」になる） |

判定はグループ見出しごとに行い、**同じ見出しの中で短縮とフルが混ざらない**ようにする。
2026-08-26 以前は一律で短縮していたため、全日本社会人で「男子」が3行並ぶ、
全日本ミックスで「混合」が8行並ぶ、という状態だった。
...
- **開催前ブロック**は **`endDate >= 今日` の information がある大会**に出る。
  「今日」は `lib/highschoolInProgress.ts` と同じく**ビルド時刻**（静的書き出しのため）。
  **「その年度の結果がまだ無い」は条件に入れない**（2026-08-26 修正）。それは `details/` に
  ファイルがあるかを見ているだけで、**組み合わせだけ投入した未開催の大会**まで「結果あり」と
  扱ってブロックを消してしまう。会期が終わっていない以上その大会は未開催なので、
