...
- **男女切替**（すべて/男子/女子）で行と収録試合数を切り替える。ミックス種目にしか出ていないチームは両方に出す。
...
- **「現在の反映状況」（`ResultCoverageNotice` / ADR-007）の分母は「決勝Tの参加数 − 1」**。
  ...
  **予選リーグは分母に混ぜないが、「まだ何も反映されていない」かの判定には見る**（2026-09-18）。
  決勝Tが1試合も決着していなくても予選リーグが決着していれば、title は「結果・組み合わせ」のままで、
  文言は「予選リーグ◯試合の結果を掲載中。決勝トーナメント(全◯試合)はこれから反映します。」にする。
- **団体戦の対戦ごとの記録**（[ADR-020](../adr/ADR-020-team-match-rubber-details.md)）がある試合は、
  「対戦詳細」の各カードに「第N対戦・ペア・本数」を並べる（`MatchResults.tsx` の `TeamMatchList`）。
  **向きはカードの組から見る**（左がその組、右が相手）。勝った側を太字にし、勝敗は `sr-only` で読み上げる。
  状態は「打ち切り」「未実施」、途中棄権は「棄権」／相手側の行では「相手が棄権」
  （**本数だけでは勝敗が逆に読める**ため必ず出す）。選手名は姓名の完全一致でリンク。
  記録のある試合が1つでもあるページだけ節の冒頭に注記を出す。
...
- **実施種目のラベルは `details/` のファイルが実在するときだけリンクにする**（`getStaticPaths` が
...
- 取り込みは `npm run popular:import -- <CSV>`（`--dry-run` で確認）。集計期間は**直近28日**。
  手順は GA4 → レポート → エンゲージメント → ページとスクリーン → 期間を過去28日 → 次元を
  「ページパス + クエリ文字列」→ 表示行数を最大 → CSV → `popular:import` → コミット。
...
- ページが無くなったリンク先は落とす。足りないときは選手は固定の3人で埋め、チームは出せる件数だけ出す。
