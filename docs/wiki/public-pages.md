...
- パンくず（2026-09-14）: チームページ `/teams/[teamId]/` と年度別 `/teams/[teamId]/[year]/[gender]/` は
  「ホーム > チーム一覧 > {チーム名}（> 年度）」。以前はホームの次がチーム名だった
- トップページ（2026-09-14）: 「所属別成績」（日本体育大学・ワタキューセイモアの2枚）を廃止し、チーム一覧へのカード1枚に置き換えた。
  見出しは「チーム」から「チームを探す」に変えた（名詞1語だと他の見出しと形が揃わず、カードの「チーム一覧」と重なるため。
  中身が検索の入口なので「〜を探す」にし、「カテゴリから探す」と形を揃えた）
  - Deprecated（同日夕方）: 「チームを探す」はやめ、「よく見られている選手」と同じ形の「よく見られているチーム」
    （GA4 上位3件のカード＋右下の「チーム一覧を見る」リンク）に置き換えた。詳細は下の「トップページのよく見られているページ」
...
- **リンクの出し分け**（TeamLink 規則・D-025）: (1) `/teams/[teamId]` があればチーム名をそこへ
  （STリーグ集計60件＋ `team-name-mappings.json` のキー。2026-09-14 に mapping を追加し、STリーグに出ていない
  日本体育大学（`nssu`）の行にリンクが付いた。同名は STリーグを優先）、
  無ければ (2) 高校 `/highschool/[gender]/[prefectureId]/[teamId]`・(3) 中学生 `/secondaryschool/[prefectureId]/[teamId]`・
  (4) 小学生 `/primaryschool/[prefectureId]/[teamId]` のうち**当たるものをすべて**出す（(3)(4) は 2026-09-14 追加）。
  学校系はどれも `(名前, 都道府県)` 照合。
  - 行き先が1つでラベルが要らない（性別選択中の高校・中学生のみ・小学生のみ）ときはチーム名自体をリンクにする
  - それ以外はチーム名の後ろにラベルつきリンクを並べる。高校は「すべて」表示で男子・女子を併記し、
    小中と並ぶときは「高校男子」「高校」と区別する（例: `スマイリー 中学生 小学生`、`昇陽 高校女子 中学生`）
  - 小中のページは男女共通なので、男女切替に関係なく同じリンクを出す
  - 2026-09-13 の見積もりで、小中のリンクが新たに付く行は594、複数カテゴリに当たる行は39
    （[raw/2026-09-13-idea-teams-index-junior-links.md](../raw/2026-09-13-idea-teams-index-junior-links.md)）。
    上の件数（3,861件・557チーム・701リンク）は 2026-08-11 時点で、現在は掲載4,380行・高校リンク865行（**要再計測**）
...
- 公開ページの文言は「小学校」ではなく**「小学生」**。掲載団体はほぼ全部がクラブ・少年団で、
  `小学校` で終わる団体は0件
- 都道府県ページには「全国大会での成績（ベスト8以上）」節がある（2026-09-13 追加）。
  県内の記録の列挙だけで、県同士は比べない
...
大学生カテゴリ（`/university`、2026-09-13 追加）:

- `/university/`（入口・1枚）／ `/university/pathways/[gender]/`（大学別の出身高校・2枚）の計3ページ
- 地域軸が無い（大学の prefecture は `日本学連`/`学連`）ので都道府県・チームのツリーは持たず、
  中身は「高校 → 大学」の進路だけ。大会軸・順位づけのページも持たない。詳細は [university.md](./university.md)
- 高校の学校ページでは「出身中学」と「進学先大学」を「{高校名}の選手の進路」1節にまとめた（案A）
- ナビ「カテゴリから探す」とトップの同名の見出しは 小学生 → 中学生 → 高校生 → 大学生 の学齢順
  （2026-09-14 にナビのグループ名を「特集」から、トップの見出しを「属性別成績」から揃えて変えた）
- 生成は `npm run university:pathways`（prebuild に組み込み済み）
- 実装: `src/pages/university/**`、`lib/university.ts`、`scripts/build-university-pathways.mjs`
...
- **順位づけをするページ・節を持たない**。当初あった県別ポイントは、大会ごとに県の出場枠が違い
  ...
  「県内にどのチームが収録されているか」の一覧。2026-09-13 に、全中・都道府県対抗それぞれで
  県内のベスト8以上の記録を列挙する節を足した（点数化・県同士の比較はしない。ブロック大会は含めない）
...
- **`club:verify` の単調増加チェックは制度変更後（2023年度以降）だけに要求する**（2026-09-13 に範囲を狭めた）。
  ...
  （[raw/2026-09-13-club-verify-monotonic-scope.md](../raw/2026-09-13-club-verify-monotonic-scope.md)）。
...
### 「現在の反映状況」の分母（2026-09-06 修正）

年度別結果ページの H1 直下に出る反映状況（`ResultCoverageNotice` / ADR-007）の
「全◯試合」は、**決勝までの想定総試合数**であって、データに入っている試合レコード数ではない。

分母は**「決勝Tの参加数 − 1」**で求める。シングルエリミネーションの試合数は不戦勝の有無に
関係なくこの式で決まるので、**結果が1件も入っていなくても総試合数は確定する**。参加数は
`knockoutDraw` があれば非 null の席数（予選リーグ→決勝T形式。`entries` にはリーグだけの組も
含むため数えない）、無ければ `entries` の数。予選リーグ併用で `knockoutDraw` が無い大会だけは
根拠が無いので算出せず、従来どおりレコード数で代用する（実測3ファイル）。

**レコード数を分母にしてはいけない。** 試合を実施したぶんだけ `matches` に追記していく大会では
分母が進行と一緒に増えるため、進捗率が常に高く出る。2026-09-06 まではこれが原因で
全日本学生2026 女子ダブルス（332エントリー＝決勝まで331試合）が「全76試合」＝1回戦の数を
表示していた。組み合わせ投入時にブラケット全体を作る大会では正しく見えるので、
**大会ごとに信頼度が違う**という気付きにくい状態になっていた。
経緯: [反映状況の分母修正](../raw/2026-09-06-coverage-denominator-fix.md)

### 中止（開催されなかった回）の見せ方（2026-09-05 追加）

`information` に `status: 'cancelled'`（→ [data-model](./data-model.md)）を持つ年は、
**結果が無い年として落とさず「中止」と明示して並べる**。回次（第N回）だけが進んだ年を
落とすと、年表の穴が「未収録」と区別できない（例: 全日本ミックスダブルスは第1回が中止のため
サイト上は第2回から始まっており、理由がどこにも書かれていなかった）。

出す場所は3つ。いずれも結果ページへのリンクは張らない。

| 面 | 見せ方 |
|---|---|
| 大会一覧 `/tournaments/` | 「中止」バッジ（**日付判定より優先**）＋結果列を「中止」にする。年フィルタで2020・2021を選ぶと中止の回が並ぶ |
| 大会ハブの「年度別結果」 | 年度降順の並びに合流させ、`{年}年度` ＋「中止」バッジ、本文で回次を明示 |
| 歴代優勝者の表（ハブ／高校歴代ページ） | その年を**列**として追加し、全種目のセルを「中止」にする |
| 選手ページの主要タイトル表 `/players/[id]/results` | その年のセルを「中止」にする（既定の `ー`＝出場なし・記録なしと区別する）。詳細は [players-pages](./players-pages.md) |

文言のルール:

- 「中止」の一語で、**理由は書かない**（打ち切りと同じ方針）。絵文字は使わない。
- 日程・会場は**実績と同じ書き方をしない**。実績は「開催地: / 日程:」、
  中止は「開催予定: 〜 / 県名」（値は中止時点の予定であって実績ではないため）。
- 年表の本文は「第75回 … は中止となり、開催されませんでした。」＝**回次を出す**
  （回次の飛びを説明することがこの表示の目的）。

**注意（未来判定）**: 中止の回は**過去日付だが `endDate` を持つ**。「結果が無い年」の意味が
未収録／開催前／**中止**の3つになったので、`endDate >= today` や `!resultYears.has(year)` で
「これから開催」を判定している箇所は必ず `isCancelledEntry()` で除外する。除外漏れがあると
中止の回が「開催予定」として並び、高校歴代ページのように日付を見ていない経路では
`SportsEvent` の JSON-LD が `eventStatus: EventScheduled` で出て構造化データに嘘が入る。
中止の回は歴代優勝者の `ItemList` にも入れない（`championRows` は `details` 由来のまま）。

`/tournaments/major/` は「結果がある年度だけ」を出す面なので中止の回は出さない
（下記「`/tournaments/major/` は『収録大会の一覧』」の方針どおり）。
...
- **年度別結果ページ（`/tournaments/[…]/[year]/[…]`）は結果がある年だけの面**で、開催前の年には
  URL を作らない（上記「新規URLは増やしていない」）。したがって**年度・カテゴリ切り替えの候補は
  `details/<tid>/<year>/<categoryId>.json` の実在で絞る**（2026-09-05 修正）。
...
#### 公式発表された代表名簿（2026-09-08 追加）

上の2節は「予選会に出場した／上位に進出した」までしか書けなかった。**公式の代表名簿が発表された
大会に限り**、名簿を一次データとして持ち込んで「日本代表に選出されている」と書く。
データの形は [data-model.md](./data-model.md)「代表名簿（`delegations`）」。

- **名簿は当サイトの導出物ではなく、外部発表の転記**。`source` / `sourceUrl` / `announcedOn` を
  データ側に必ず持たせ、描画側で併記する（読者が一次情報を確認できるようにするため）。
  逆に**名簿に書かれていないことは足さない**（混合ダブルスのペア構成など）
- **名簿がある大会では予選会の上位進出者を出さない**（計算もしない）。予選会はシングルスのみで
  団体・混合は別経路のため、上位4人には代表でない選手が混ざる（アジア競技大会2026では8人中3人）。
  併記すると誤読を招く。名簿が無い大会は従来どおり予選会ブロックが受け持つ
- **名簿は予選会と独立の入口**。予選会に出ていない選手でも名簿にあればブロックが出る
  （団体・混合は予選会を経ずに選ばれ得るため）。この経路では予選会の成績欄は空になる
- **名簿の年が「これから開催される回」と一致するときだけ採る**。4年周期の大会では前回の名簿が
  残るため、予選会の「回」を絞るのと同じ理由・同じ形
- 選手ページでは、名簿がある場合に**予選会の成績を併記しない**。団体・混合は予選会を経ずに
  選ばれるので、成績を並べると選考の根拠であるかのように読める。予選会へのリンクだけ残す
- 検査は `npm run check:upcoming` の **[4]**（氏名が選手ページに解決できるか＝表記ゆれの検出、
  `categoryId` が information にあるか、対応する開催情報があるか）

#### 種目別の日程（2026-09-17 追加）

会期だけでは「いつ見ればいいか」が分からないため、主催者が種目別の日程を出している大会では
それを出す。データの形は [data-model.md](./data-model.md)「種目別の競技日程（`schedule`）」。

- **大会ハブ**: 開催前ブロックの「実施種目」と「会場」の間に「種目別の日程（予定）」表
  （種目・日程・決勝開始）。並びは開始日 → 決勝日 → 決勝時刻。`SportsEvent.subEvent` にも日付を載せる
  （時刻は載せない）
- **選手ページ**: 代表名簿に載っている選手だけ、**その選手の出場種目の日程だけ**を出す。
  名簿に無い種目の日程を並べると出場するように読めるため
- 日付は曜日つき（`9月18日（金）〜20日（日）`）。スマホ幅では「〜」の後でだけ折り返す
- 出典・確認日・「予定」であること・時刻が会場の現地時刻であることを必ず併記する
  （日本時間と書かないのは、国外開催の国際大会にも同じ部品を使うため）
- 出典の無い `schedule` は表示しない

**残作業**: 順番つきの一覧を
[開催前の大会・国際大会の露出 実行ランブック](./upcoming-tournaments-runbook.md)に置いた。

### トップページ（`/`）の並び（2026-09-14）

実装: `src/pages/index.tsx`。サイドナビ（`lib/navigation.ts`）のグループ順に揃える。

1. サイト紹介（h1）
2. これから開催
3. 最近追加された大会
4. よく見られている選手
5. よく見られているチーム（2026-09-14 夕方までは「チームを探す」）
6. 選手ランキング
7. カテゴリから探す（小学生・中学生・高校生・大学生・STリーグ）

- 2〜6 がナビ「成績・記録を調べる」（大会 → 選手 → チーム → ランキング）、7 が「カテゴリから探す」に当たる
- STリーグは 2025-12 の公開時から「ソフトテニス情報」見出しの直下（ページ本文の先頭）にカード1枚で置いていたが、
  2026-09-14 に「カテゴリから探す」の5枚目へ移した。ナビでは同グループの一項目で、トップだけ先頭にある理由が無くなったため。
  あわせて中身の無くなった「ソフトテニス情報」見出しを外し、カードの説明文をキャッチコピー
  （「ソフトテニス実業団最高峰の戦い」）から掲載内容の説明に変えた
- 「これから開催」の見出しは、トップだけ枠の外に他のセクション見出しと同じ大きさ（`text-xl`）で出す
  （`UpcomingTournaments` の `headingSize="section"`、2026-09-14）。STリーグのカードを外して本文の最初の見出しに
  なったため。大会一覧はファーストビューの件数制約があるので、枠の中に小さく出す従来の形（`compact`）のまま
- Assumption: STリーグの開催期（冬）に先頭へ戻して目立たせるかは未検討。アクセス数での比較もしていない

### トップページのよく見られているページ（2026-09-14）

「よく見られている選手」と「よく見られているチーム」に、**GA4 の閲覧数の上位3件**を出す。
2つは同じ形（h2・説明文と集計期間・カード3枚・右下に一覧へのリンク）。チームは当初「チームを探す」の中の小見出しだったが、
同日にユーザー要望で独立したセクションにした。
それまでは選手3人（上松・上岡・丸山）を手で選んで固定していた。リアルタイムではなく、**手で取り込んだ時点の並び**。

- データ: `data/popular-pages.json`（コミットする）。リンク先の並びだけを持ち、**表示回数は書かない**（リポジトリが公開のため）
- 取り込み: `npm run popular:import -- <CSV>`（`--dry-run` で表示回数つきの順位を確認するだけ）。
  実装は `scripts/import-popular-pages.ts`、集計は `lib/popularPages.ts`（テスト `npm run popular:test`）、
  名前の解決は `lib/popularPagesData.ts`
- 集計期間: **直近28日**（2026-09-14 決定）。1週で入れ替えても順位が暴れにくいため
- 数えるページ
  - 選手: `/players/{id}/results/`。プロフィール `/players/{slug}/` は同名が1人に決まるときだけ結果ページへ合算
  - チーム: `/teams/{id}/`（年度別ページも合算）、高校の学校ページ（男女別に別件）、中学・小学生のチームページ
    （2026-09-14 決定: 学校ページも含める）。カードは実際に見られたページへリンク
  - クエリ文字列の違いは同じページとして合算。ただし **`?q=` 付きと `#google_vignette` 付きの行は数えない**（2026-09-14、実物の CSV で判明）。
    `?q=` は検索窓の1文字ごとの入力で、`/players/4898/results/` の141回のうち46回がこれ（中身は別の選手名）。
    `#google_vignette` は AdSense のビニエット広告が付けるハッシュで同じ閲覧の重複。
    Assumption: 選手ページの URL に `?q=` が付く経路は未特定（ローカルで「検索 → 結果ページ → 戻る」では再現しない）
- ページが無くなったリンク先（掲載閾値を下回った等）は取り込み時とビルド時の両方で落とす。JSON には控えとして10件ずつ残す
- 足りないとき: 選手は固定だった3人で埋める。チームは出せる件数だけ出し、0件ならカードを出さず見出し・説明文・「チーム一覧を見る」リンクだけ残す。
  集計期間の文言（「〜の閲覧数をもとにしています」）は GA4 由来の並びを出しているときだけ出す
- カードの補足: 選手は所属（`players-search.json` の最新所属）、チームは「STリーグ」「大学」「高校男子・奈良県」「中学生・東京都」など

**更新手順（週1回・手動）**

1. GA4 → レポート → エンゲージメント → ページとスクリーン。期間を「過去28日間」にする
2. 表の上の次元を「ページパス + クエリ文字列」（または「ページパスとスクリーン クラス」）にする
3. 表示行数を最大（右下）にしてから、共有 → ファイルをダウンロード → CSV
4. `npm run popular:import -- ~/Downloads/<ファイル名>.csv --dry-run` で上位を確認し、問題なければ `--dry-run` を外して実行
5. `data/popular-pages.json` をコミット・push すると、次の本番ビルドで入れ替わる

注意:

- 2026-09-09 より前の期間には開発中の自分の閲覧（`next dev`）が混ざっている（[ADR-018](../adr/ADR-018-consent-by-region.md) と同日に除外）。
  標準レポートの CSV ではホスト名で除けないので、取り込みスクリプトが警告だけ出す。2026-10-07 以降の取り込みでは該当しない
- CSV の形（2026-09-14 に実物で確認）: 探索（自由形式）の書き出しは `ページパス + クエリ文字列,表示回数` の2列で、
  期間は冒頭の `# 20260817-20260913` の1行、総計行（`,54334,総計`）を含む。標準レポートの `# 開始日:` 形式も読めるようにしてある
- 同 CSV では**表示回数 54,333 回のうち 19,883 回（36.6%）が `?q=` 付き**の行だった（大半は `/players/?q=`）。
  `_app.tsx` が検索窓の入力による shallow な URL 更新でも `page_view` を送るため。サイト全体の PV や
  回遊検証のモジュールCTR（分母が `page_view`）を読むときもこの分を差し引く必要がある（[Open Questions](./open-questions.md)）
- 自動化（GA4 Data API ＋ GitHub Actions で週1）は未着手。[Open Questions](./open-questions.md) 参照
...
- **画像に大会名・年・種目は入れない**。X はカードの下にページタイトル（「{大会名} {年}年
  ...
  2回出る。フッターも置かず、縦をすべて表に使う。
- **上部のサイト名バーは廃止した（2026-09-09 変更）。** 2026-07-31 には「サイト名だけを上に置く」
  としていたが、**X はカードの左下にタイトルを重ねて表示する**ため、下に余白が無いと表の
  一番下の行が隠れる。バーの高さ（ネイビー52px + 下線のイエロー5px = **57px**）を
  **そのまま下の余白へ振り替え**、表全体を57px上へ寄せた。
  - `bottom - top` は525pxのまま変えていないので、**表の縮尺・行間・可読性は従来と同じ**。
    実測で描画領域は「上37px / 下**94px**」（従来は上0px＝バー / 下37px）。
  - サイト名の**文字**は置かない。代わりに**右下にブランドマークだけ**を置く（2026-09-09 追加）。
    X のタイトルは左下から始まり、**長いと右端近くまで伸びる**ので、文字を足すと二重になったり
    重なったりする。マークだけなら衝突しない。
    - 素材は既定 OG カード `public/og/twitter-card-summary.png`（192×192）から
      **マークだけを切り出す**。インクのある行を数えると 18〜144（マーク）と 153〜170
      （「Softeni Pick」のワードマーク）の2ブロックに分かれるので、前者だけを取る。
    - 元画像は地が `(239,239,235)` の**不透明** PNG なので、そのまま貼るとキャンバスの地
      `(250,250,248)` との差が四角い枠として見える。**地の色からの距離をアルファに変換して
      切り抜く**（二値マスクにすると縮小時に縁がギザつくため、距離ベースにしている）。
    - 一辺38px・右24px・下20px。右マージンはトーナメント表の左右マージンに合わせている。
      定数は `BRAND_MARK_*`。
  - 実装は `tools/sns-images/tournament_og.py` の `BRAND_BAR_H` / `TOP_MARGIN` /
    `BOTTOM_MARGIN` と `render()`。振り替えの意図がわかるよう、**バーは消しても
    `BRAND_BAR_H` は定数として残している**。
...
- 生成: `.venv/bin/python tools/sns-images/tournament_og.py --apply`（Pillow、`snslib.py` の
  ...
  - **通常は `--changed`**（2026-09-09 追加）。`data/tournaments/details/**` の更新から対象を
    ...
    対象は実行時に一覧表示される。
  - **`--changed` は未追跡（`git status` の `??`）のファイルを拾わない。**
    新しい年をまるごと取り込むとディレクトリごと未追跡になるため、
    決着済みでも「対象0件」と出る。インカレ2022の3種目を取り込んだ 2026-09-11 に遭遇
    （[記録](../raw/2026-09-11-incare-2026-singles-x-post-findings.md)）。
    **新規に取り込んだ年は `--only <tid>/<year>` で明示する**か、先にコミットしてから回す。
  ...
  - 手で絞るときは `--only <tid>` / `<tid>/<year>` / `<tid>/<year>/<categoryId>`。
    ...
    **年で絞っても前方一致は消えない**（`--only zennihon-university/2022` が対象6件で
    済んだのは、姉妹大会にたまたま2022年が無かっただけ）。
    **実行時に出る対象件数が想定と合うことを毎回確認する。**
  - **団体戦（`versus-*`）にも生成される。** 学校名で描いたトーナメント表になる。
    「ベスト16のトーナメント表」という説明からは読み取りにくいが、個人戦専用ではない。
...
- **全件 `--apply` は既存の画像もほぼ全部差し替える**（2026-09-09 実測: 337枚中 **313枚**）。
  ...
  **中身が同じでもハッシュが変わる**。画素差を実測したところ 10.1% のピクセルが変化していたが、
  ...
  - **`--changed` を通していればこの問題は起きない**（更新された種目しか触らないため）。
    ただし**描画そのものを変えたとき**（レイアウト変更・配色変更）は全件作り直しが正しいので、
    そのときだけ `--all` を使う。2026-09-09 のサイト名バー廃止では456枚すべてを差し替えた
    （データではなく見た目が本当に変わっているので、これは churn ではない）。
...
| アイデア | 状況・目的（1行） | 詳細 |
|---|---|---|
| トップの「よく見られている選手・チーム」を GA4 の実績から出す | **実装済み（2026-09-14）・手動取り込み版**。GA4 の CSV を手で流して上位3件を出す。週1の自動化（GA4 Data API＋Actions）は手動運用を試してから判断。未決は [open-questions.md](./open-questions.md) | [raw/2026-09-14-idea-top-popular-pages-ga4.md](../raw/2026-09-14-idea-top-popular-pages-ga4.md) |
| チーム一覧（`/teams`）から小学生・中学生のチームページへリンク | **実装済み（2026-09-14）**。TeamLink 規則に中学生・小学生を足し、複数カテゴリに当たる行は**ラベル付きリンクを並べる**方式（ユーザー決定）。同時に mapping ページ（日本体育大学）の行にもリンクが付くよう修正。見積もりで新規リンク594行 | [アイデア](../raw/2026-09-13-idea-teams-index-junior-links.md) |
| 日本体育大学の独自ページ（`/teams/nssu/`）と大学カテゴリの関係 | **実装済み（2026-09-14）: `/teams/nssu/` は残し、収録100試合以上の大学37校を同じ `/teams/[teamId]` 型で追加（計38校・551ページ）。大学カテゴリと相互リンク**（[university.md](./university.md)）。以下は起票時の記録:大学で唯一の個別ページ。**URL は変えず `/university` と相互リンクで繋ぐ**のが推奨（大学の個別ページは今後も `/teams/[teamId]` 型に寄せる）。`/teams` 一覧で日本体育大学（929試合・1位）の行にリンクが無い不具合も見つかった（mapping ページを TeamLink が見ていない） | [アイデア](../raw/2026-09-13-idea-nssu-team-page-and-university.md) |
| 今日のソフトテニスライブ配信一覧（`/live/`） | **2026-09-06: 発散フェーズ・未着手（起票のみ）**。YouTube Data API で今日開催の大会に紐づくライブ配信を自動収集して出す案。既存構成に照らして評価した結果、**3つの制約が先に効く**。(1)**供給量が未知**——「ソフトテニスの大会がどれだけ配信されているか」をこのリポジトリは1件も持っておらず、機能の価値はそこだけで決まる。よって最初の一歩は実装ではなく**収集だけして掲載しない dry-run**（`checks.yml` の「報告のみ」と同じ置き方）。(2)**空の日が常態化する**——`information` 全164レコードの実測で **2026年に大会が登録されている日は100日/365＝27.4%**（同日最大6件、起票日9/6は全日本学生選手権の1件）。年間7割の日は「今日の大会」が0件になるので、`/live/` を配信一覧として作ると空ページになる。seo.md #11（会期中は**新規URLを作らず**既存の指名済みページを更新）と ADR-010（薄い/重複ページを作らない）の先例に沿うなら、まず大会ハブ・年度別結果ページへの「ライブ配信」ブロック（新規URLゼロ）、`/live/` は「今日のソフトテニス」入口として供給確認後に判断。(3)**鮮度が静的エクスポートと衝突する**——`output: 'export'` でランタイムが無く、`prebuild` は重い（seo.md #11 の既知課題）。かつ `review-snapshot.yml` が「**生成物を書き戻すな**」を原則にしているため `live.json` のコミット＋再ビルドは採れない。ビルド時生成（`public/data/live.json`・既存の生成物と同じく git 非追跡）／CF Pages Functions+KV（**ADR 相当のアーキテクチャ変更**）の二択で未決。MVP は**リアルタイムの「LIVE中」を名乗らない**方針を推奨（根拠の持てない状態表示をしないほうが「存在しない配信を載せない」という提案の趣旨に合う）。quota は既定10,000/日に対し、**発見は日次 `search.list`（100 units）・状態更新は `videos.list`（1 unit）**に分ければ約650 units/日で収まるが、**30分ごとに search を回すと28,800 units で破綻**する。`YOUTUBE_API_KEY` はこのリポジトリ初の Secret になる。なお提案文中の `🔴 LIVE` / `📺` は**絵文字禁止（eslint で error）**のためそのままでは通らない。**同日、精度と収益も見積もった**——(4)**大会名が競技を特定しない**。2026年の延べ開催日156日のうち名前に「ソフトテニス」を含むのは**15日（9.6%）**しかなく、**37.8%（59日・19大会）は他競技と名前を共有する総合大会名**（インターハイ・全中・各ブロック大会＝**配信される見込みが最も高い大会がここに集中し、他競技の同名大会と会期まで重なる**）。これは seo.md「大会名の表記と検索語の乖離」で実証済みの構造に検索の向きを変えて当たっているだけ。クエリに「ソフトテニス」を足すと再現率が落ち、略称マスタ（`searchLabel`/`searchAliases`）の流用は**31大会中3大会しか設定が無い**。本命はチャンネル起点だが既知チャンネル一覧を持っていない。(5)**`/live/` は広告面として成立しにくい**——`getPageType()` に種別を足さないと `other` 扱いで**広告ゼロ**、かつ `FOOTER_AD_MIN_CONTENT_CHARS = 400` に対し配信1件の本文は80〜100文字程度なので**常時3〜4件無いと枠が出ない**。さらに `/live/` は**リンク先が youtube.com の離脱ページ**で、seo.md #11 が問題として直した「訪問者を外部へ逃がす」挙動と同型であり、回遊＝PV増＝収益という目的関数と向きが逆。増える広告面も**+1枚**（フッター枠展開は433→約3,900枚だった）。**B1（大会ハブ・年度別結果ページへの差し込み）と B2（`/live/` 新設）は同じ機能の2つの見せ方ではなく逆方向の施策**として扱う。B1 は差し込み先が既に広告面で会期中は需要のピーク（seo.md #11 で実証済み）。売上の絶対額はリポジトリに無く（monetization.md は相対値のみ）、AdSense のページRPMと GSC の「ライブ/配信/中継」クエリを見れば出せる | [アイデア](../raw/2026-09-06-idea-live-streams.md) |
| ユーザーの課題を体系的に見つける | **2026-09-06: 発散フェーズ・未着手**（棚卸しのみ済み）。これまでの課題発見は「データが壊れている」型に偏っており、**ユーザーが使いにくい・欲しいものが見つからない**型は一度も体系的に探していない。既存のUX改善（一覧表現・節の階層化・広告位置）はすべて作り手が気付いた範囲で、ユーザー側の信号から出発したものが無い。**信号の棚卸しを実施**——観測できるのは (a)GA4のサイト内検索クエリ (b)GSCの低CTR/11〜30位クエリ (c)GA4の`internal_link_click` (d)`out/`の静的解析 (e)`/contact`の声。観測**できない**のはセッション単位の指標（同意がデフォルト拒否で`client_id`が不安定）とページ内操作（未計測）。**※2026-09-09 追記: 日本からのアクセスは同意バナー廃止で granted になり（[ADR-018](../adr/ADR-018-consent-by-region.md)）、セッション単位の指標はこの日以降のデータについては読めるようになった。この日より前のデータには当てはまらない。また同日まで`next dev`も本番プロパティへ送っていたため、**2026-08-09〜09-09のクエリには自分のdevブラウジングが混ざっている**——手順1では GA4 の`hostname`ディメンションで`localhost`を除外すること。2026-09-09以降のデータにこの問題は無い。****最有力は(a)**——`/players`は検索語をURLに反映しており（2026-08-09〜）、`_app.tsx`が`page_path`にクエリ文字列ごと`page_view`を送るため、**「ユーザーが自分の言葉で何を探したか」が約1ヶ月分GA4に溜まっている。しかもGA4は一度も読まれていない**（[回遊検証ランブック](./circulation-verification.md)）。検索インデックスは9,391組なので**0件になった語＝収録の穴か表記違い**で、どちらも打ち手に直結する。**手順は他人が実行できる形で記述済み**（前提確認→検索クエリ→GSC取りこぼし→行き止まりページ→優先順位→記録）。まず手順1だけを30分やる方針——既にデータがあり、ユーザー自身の言葉で、空振りでもすぐ分かるため。未確認はGA4側でクエリ文字列が落ちていないか（**空振りならここで止まる**）。**同日、0件判定を実装済み**（`scripts/check-search-misses.mjs` / `npm run check:search-misses`）——クエリ一覧を受け取り0件の語と30件超ヒットの語を出す。照合は画面と同じ実装で、GA4の書き出し（`/players?q=`・URLエンコード可）をそのまま流せる。**認証不要**。GitHub Actions での自動化は「取得（GA4/GSC の API＋サービスアカウント）」と「解釈（ローカル完結）」に分かれ、**解釈側だけ先に作った**（手順1が空振りなら取得の実装は無駄になるため）。`out/`の静的解析はビルド約9分が要るので最後。**注意: GA4のイベント保持は既定2ヶ月**で、既定のままなら2026-08-09以降のクエリは2026-10-09前後で消え始める（要設定確認）。消えるなら「再生成できない記録」として定期取得＋書き戻しの価値がある | [アイデア](../raw/2026-09-06-idea-user-problem-discovery.md) |
| 大学カテゴリの公開ページ | **実装済み（2026-09-13）。** `/university/`・`/university/pathways/[gender]/`（大学別の出身高校）を新設し、高校の学校ページの出身中学と進学先大学を「進路」1節にまとめた（案A）。ナビ「特集」に「大学生」を追加。年差は実測で4年、推定誤マッチ約1%。仕様は [university.md](./university.md)、検証は [raw/2026-09-13-university-pathways-verification.md](../raw/2026-09-13-university-pathways-verification.md)。**以下は保留（2026-08-14）までの経緯**（数値は当時のもの）。2026-08-11 の成年カテゴリ検討で「大学は都道府県が0.5%しか付かないので向かない」と一度棄却したが、**深さを測っていなかった**ため見直した。実測で**大学は中学の真逆のプロファイル**——チーム数は少ないが**1チームが濃い**。`日本体育大学`は全チーム中1位なのに `/teams/[teamId]` の `getStaticPaths` が `team-name-mappings.json`（大学0件）＋STリーグのみなので**大学は1校もページが無い**。**2026-08-14、中学カテゴリの実装を踏まえて方針転換**: ツリーを作るのではなく**進路接続を先にやる**。理由は(1)`prefecture` が `日本学連`/`学連` で**地域軸が本当に存在せず**、ツリーを作っても135ページの平屋にしかならない (2)進路なら**新規URLは索引2枚だけ**で残りは高校の既存721ページへの節追加 (3)**高校×性別グループ300は中学の111の2.7倍**（高校ページの42%に付く。進路採用1,111件・受け皿123大学・3名以上を送り出す高校×性別122）。中学の実装からの学びとして**ランキングは作らない**（中学は県別ポイントを実装後に廃止。大学は深さの相当部分が一般大会由来なので分母がさらに恣意的）、**大会軸ページも作らない**（ADR-010、インカレ・王座は既存ハブへ）。**中学→高校→大学の3段接続は現時点で0人**（収録年が足りない）だが、**インカレ2026（例年8月末〜9月初旬）の結果投入で初めて成立する**。**着手前に節の並べ方の決定が要る**（2026-08-14）: 進学先大学を足すと高校の学校ページに「主な卒業生」66グループ・「出身中学」113・「進学先大学」300の3節が並び、**実測で96人＝主な卒業生の69%が二重に出る**（卒業生140件のうち55%は最後の所属が大学）。「出身中学」と「進学先大学」は同じ進路データの前後なので1節にまとめる案Aを推奨。詳細は[節の整理の設計](../raw/2026-08-14-highschool-pathway-sections-design.md)。**チーム名の下ごしらえは適用済み（2026-08-14）**: 186→163チーム、中央値20→25、試合5未満25.8%→16.6%。初回の「表記ゆれほぼ0」評価は誤りで、壊れ方が先頭文字脱落型のため通常の正規化をすり抜けていた。**大学名は大学大会の外にも現れる**ため専用大会だけを見た棚卸しでは取りこぼす、という教訓つき | [アイデア](../raw/2026-08-12-idea-university-category-pages.md) / [チーム名の下ごしらえ](../raw/2026-08-12-university-team-name-cleanup.md) |
...
| トーナメント表の作り直し | **実装済み・ビルド確認済み**（2026-07-31実装、2026-08-07 `npm run build` 実機確認・問題なし）。`TournamentBracket` は `describeBracketLayout` が成功すれば新描画 `BracketSheets`、失敗すれば従来描画にフォールバックする（`nextMatchId` の有無では分岐しない）。モジュールは `lib/bracketLayout.ts`（純粋関数）／`lib/bracketLayout.server.ts`（`fs` を使うので分離）／`lib/bracketDrawing.ts`（座標計算。SVG文字列ではなく配列を返しモックとReactで共有）／`src/components/Tournament/BracketSheets.tsx`。**`lib/bracketLayout.ts` は `tournamentRecords` から型だけを import すること**（`fs` が値で入るとクライアントバンドルが壊れる）。以下は設計の経緯。本命 A は**結果が未入力でもブラケットの全ラウンドと接続線が描かれていて、勝ち上がりを目で辿れる**こと。**勝者を選ぶインタラクションではないので状態管理は不要**（ユーザー確定。SSG のまま完結する）。あわせて大規模ドローの分割表示（B）と左右対称レイアウト（C）を検討したいが、これらは A と同じツリーへの別の見せ方に還元される。現行 `TournamentBracket.tsx` は `matches` のツリー（`nextMatchId`）前提のため**開催前は成立せず**（IH2026は`nextMatchId`が0件で60本の独立ツリーになる）、シード・足長も表示されない。復元は `lib/bracketLayout.ts`（前哨戦用に先行して切り出し済み）にあり、**全データ検証で209大会・20,571試合が一致／不一致0件**（`npm run bracket:verify`）。検証の副産物として不具合を2件修正: **(1) `type` の入力ミスで席がずれた10大会が誤ったラウンドを表示していた**（検出したら復元を諦める＋`bracket-slot-parity` ルール追加）、**(2) bye無しの2冪ドロー36大会を「シード未入力」と誤判定して取りこぼしていた**。復元できない残りは予選リーグ81（構造的に対象外）・`type` null の完了済み15（逆算可能・未実施）・席ずれ10。**描画用ツリー `buildBracketTree` は実装・検証済み**（209大会・20,571試合が正しいラウンドに配置、欠落0。`npm run bracket:verify:tree`）。IH2026（結果未入力）で512枠・9ラウンドがすべて揃うことを確認。**B・C の基準も確定**（2026-07-31）: **1枚64枠**（＝ベスト64が1枚に収まる）で、山シートは各山の代表が決まるまでの6ラウンド、それとは別に**ベスト64シート**を持つ（4〜6回戦が重複するのは承知の上）。各枚は**左右から中央へ収束**し、拡大縮小できること。512枠は9枚、64組以下の大会（210中124）は1枚で完結。描き方は**紙のドロー表と同じく選手名を左右の両端にだけ置き、内側は線だけで繋ぐ**（勝ち上がった選手を各ラウンドに書き直さない。勝者は線の濃さで示し、**縦線も勝者側の半分を太く**して横→縦→横が1本に繋がるようにする）。これにより64枠が**602×682px**に収まる。線を引くかどうかは `BracketNode.present`（その山にエントリーが居るか）で判断すること。`entries`（確定した entryNo）で判断すると**結果未入力の大会で線が消える**。不戦勝の枠は縦線を引かず、その選手の高さのまま次ラウンドへ通す（中点に寄せるとシードの線が折れる）。また**不戦勝で通した区間は、その先で勝った時点に遡って太くする**（シード・足長の太線が選手名の根本から始まるようにするため。引いた直後には太さが決まらないので、線は配列に貯めて後からまとめて出力する）。**ラウンド名の見出しは出さない**（列幅に対して長すぎて隣と重なる。紙のドロー表にも無い）。**エントリー番号は名前のさらに外側**、**スコアは上下それぞれの獲得ゲーム数をその側の横線の上に1つずつ**置く（`4-2` と1つにまとめると常に勝者が左に来てどちらの組の点か読めないため。勝者は濃く太字、敗者は薄く、棄権は敗者側に「R」）。**決勝は中央の左右に振り分ける**。**決勝の横線は両側とも中央まで引くが、太いのは勝った側だけ**（他のラウンドと違い左右が同じ高さでぶつかるため、両方太くすると1本の長い太線になりどちらが勝ったか読めない。負けた側は細い線で決勝まで来たことを示す）。**負けた枠へ向かう区間も太くする**（太線は「どこまで到達したか」を示すので、勝ち上がった先で負けた線が前の枠の中点で途切れて宙に浮かないように）。ただし**その試合が実際に行われている場合だけ**（未開催の試合へ伸ばすと勝ってもいないのに勝ち上がったように見える）。線の太さの規則は目視で気付きにくいので `lib/__tests__/bracketDrawing.test.ts` で固定している。**山シートのタブ名は「第N山」ではなくその山の entryNo の範囲**（例: 「1〜39」。読者は自分の応援する組の番号で山を探すため）。**ベスト64シートは `decided`（勝敗が決まった枠数）が0なら非表示・1以上なら初期表示**（山シートと重複するため、結果が無いうちは出す意味が無い）。**復元できない大会は従来描画にフォールバックする**（2026-07-31 時点で229大会が新描画・87大会が従来描画。内訳は予選リーグ81＝構造的に対象外、entryNoが連番でない6）。`type` が未入力・入力ミスだった19大会は `node scripts/backfill-entry-type.mjs --apply` で `matches` から逆算して埋めた（逆算結果を信じず、全knockout試合が正しいラウンドに配置されるか検算してから書き込む）（`nextMatchId` があれば従来の `buildBracket()`、無ければ席順復元の二段構え。`type` の逆算によるデータ一本化はしない）。モバイルの初期表示は第1山でよい（エントリー番号があるので自分の山を引き算で探せる）。`splitBracketSheets()` として実装済みで、モックは `npm run bracket:preview`。**`TournamentBracket.tsx` の差し替えまで実装済みで、`npm run build` も2026-08-07に確認済み。残タスクなし**。**2026-08-22: 予選リーグ→決勝T形式の大会（90ファイル）も新描画の対象になった**。この形式では決勝Tの席は**エントリーではなく予選リーグの組に属する**ため `entries[].type` では表せず（`entryNo` 順に積むと 90 大会中 17 大会が誤ったブラケットになる）、席順を `knockoutDraw`（(組, 組内順位) の並び）として details JSON に持たせた（[ADR-015](../adr/ADR-015-knockout-draw-by-group.md)）。**復元適用 285 → 372 大会・突合 26,527 → 27,633 試合で不一致0件**。生成は `npm run bracket:draw -- --apply`（`matches` から起こし、検算を通ったものだけ書き込む）。入力ツールも保存時に `knockoutDraw` を出力する（手順の実体は `tools/shared/knockout-draw.js` を共有）。決勝が1試合だけの大会（リーグ→リーグ→優勝決定戦）はブラケットが無いので対象外。開催前でも「A組1位 vs D組2位」として表を出せる土台になったが、未確定席の見せ方は未設計で現状は空席として描かれる。**2026-09-01 修正: 「決勝は左右が同じ高さでぶつかる」という前提が誤りだった**。`leafYOf` が不戦勝の空席ぶんの行を確保しない（2026-08 のユーザー指摘への対応）ため、不戦勝の位置が左右で違うと決勝の高さがずれる——実測で**全1,002シート中461枚＝46.0%がずれており、最大24.2px**。決着点（優勝の丸）を左右の**中点**に置いていたので、ずれる大会では丸がどちらの線にも接しない宙ぶらりんの点になっていた（全中2026 男子団体で ly=191.06 / ry=177・中点184.03）。修正は**描く前に左右の高さを揃える**方式にした（`convergedYOf` で両側の収束点を先に測り、低い方の側を丸ごと下げてから描く）。これで「決勝は左右が同じ高さでぶつかる」という描き方の前提が常に成り立つので、決勝の線・スコア・優勝の丸に特別扱いが要らなくなる。**段差を線で吸収する方法（決勝の横線を決着点の高さで引き、ずれる側はラウンドの境目で縦に繋ぐ）も試したが不採用**——準決勝を勝った太線が元の高さで終わって突き出し、縦線の終点が横線とずれるため。**全1,002シートで検算し、決勝の横線2本が違う高さ0件・丸がどちらかの線に乗っていない0件・太い縦線が決勝の横線から突き出ている0件**。**2026-09-06: 本戦の前に「予選」を1試合だけ持つ形式に対応した**（実例は `zennihon-singles/2017/singles-none-boys` の257名。bye を増やして512枠にする代わりに予選で1席を争わせる）。予選の敗者は**本戦のドローに席を持たない**ため `seed`/`extra`/`packing` のどれでも表せず、`entries[].type` に `preliminary`（枠を消費しない）を追加した。**この対応前は512枠の誤ったトーナメント表を公開していた**（`describeBracketLayout` が成功してしまい新描画に乗るため）。予選の1試合はブラケット表には描かれない（本戦に席が無いので構造的に描けない）。同時に highschool-championship/2013 男子ダブルスの試合重複（同じ組が entryNo 136 と 277 に二重登録され、277の2試合が複製されていた）を修正。**復元適用 443 大会・突合 38,729 試合で不一致0件**（`npm run bracket:verify`。この照合は `.github/workflows/checks.yml` のゲートに入っている）。経緯は [調査メモ](../raw/2026-09-06-bracket-verify-250-mismatches.md) | [アイデア](../raw/2026-07-26-idea-bracket-redesign.md) |
| 一覧表現の方針（カード／リスト／表）と大会結果ページ | **大会結果ページのみ実装済み（2026-09-01）・サイト全体の方針は未確定**。出発点は「全体的にカード型だが、リストが多くなってきた」。調べると `05-page-structure.md` §6 に一覧表現の基準が2026-07-04にapprovedで存在し、**段階適用の側が未実施**だっただけと判明（新しい基準を作る問題ではない）。375×812の実測でカードもテーブルも桁は変わらず（`/tournaments` 24画面・カード／`/players` 37画面・テーブル）、長さを決めていたのは**1行の情報量・全行に繰り返す装飾・本文前の統計カードの縦積み**の3つだった。**最も見られる大会結果ページで先行実施**。インターハイ2026男子ダブルスの解剖でトーナメント表は全体の**3%**、対戦詳細316組が**89%（38.7画面）**を占め、`MatchGroup` が閉じていても**見出し行は常時描画**されるため「見たい人が見る」形式になっていなかった。導線は `main` 内99リンク・ユニーク選手66人（出場632人の10.4%＝`count >= 5` の構造的上限）で、**38画面で66リンク**。しかも `BracketSheets.tsx` に `<Link>` が0件で、モーダル経由の導線が**モバイルではタップ可能と分からない**状態だった。**実施したのはA（トーナメント表に点線の下線＋説明文、モーダルへ学校ページリンク追加）とB（対戦詳細を `<details>` で既定折り畳み・上位を成績順で常時表示・検索中は自動展開・24組以下は畳まない・`<details>` 自体は囲まない＝カードの二重を避ける）**。点線は **SVG が `text-decoration-style` を無視する**ため `strokeDasharray` の `<line>` を自分で引いており、必要な文字幅は `getComputedTextLength()` が SSG で使えないので文字種からの見積もり（半角0.5em / CJK 1em）で出している（3大会・176件で実測と不一致0件）。C（注目ポイントを結果より前に出す順序変更）はユーザー判断で見送り。**結果は 35,384px（43.6画面）→ 5,412px（6.7画面）＝-85%、選手リンクは66本のまま変化なし**（`<details>` はHTMLに全件残すため）。`npm run build` 完走・結果ページ427枚中330枚（77.3%）が畳み対象。残タスクはDOM要素数の削減（`<details>` は閉じていても子をDOMに持つ）・モジュールCTRでの効果測定（ベースラインは2026年9月末）・残りの一覧ページ（統計カードのモバイル横並び → `/tournaments` → 高校の都道府県ページ → pathways） | [アイデア](../raw/2026-09-01-idea-list-representation-and-result-page.md) |
