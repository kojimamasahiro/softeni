...
- 見出しは**大会名と「年 種目」を同じ大きさで2行**。大会名は `index.json` / `local_index.json` の
  `label` を使う（`information` の `source` は出典 PDF の名称であって大会名ではない）。
