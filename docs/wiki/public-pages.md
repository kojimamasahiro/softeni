...
| アイデア | 状況・目的（1行） | 詳細 |
|---|---|---|
| 大学カテゴリの公開ページ（/highschool 型の横展開） | **発散フェーズ（2026-08-12）**。2026-08-11 の成年カテゴリ検討で「大学は都道府県が0.5%しか付かないので向かない」と一度棄却したが、**深さを測っていなかった**ため部分的に見直す。実測で**大学は中学の真逆のプロファイル**と判明: チーム数186と少ない代わりに**試合数の中央値20（高校4.5の4.4倍・全カテゴリ最良）**、5未満は25.8%（高校50.0%・中学72.3%）。`日本体育大学`643試合は**全チーム中1位**で、収録試合トップ15のうち6つが大学。それなのに `/teams/[teamId]` の `getStaticPaths` は `team-name-mappings.json`（大学0件）＋STリーグのみなので**大学は1校もページが無い**。欠陥は「データ不足」ではなく「軸の選び方」で、大学の地域軸は都道府県ではなく**学連9地区**（王座がまさに学連代表の団体戦）。**高校→大学の接続は1,128人＝大学選手の32.4%**で、中学→高校（547人・18.5%）の2倍強い。しかも既存の高校「主な卒業生」機能はすでに大学出場を卒業後シグナルに使っており、**大学データは高校カテゴリの土台として既に load-bearing なのに着地先が無い**。弱点は大学専用大会が結果3,274行（高校の半分・収録2023年〜）で、深さの相当部分が一般大会出場由来という二重性。カニバリ懸念は中学案・成年案と違い構造的に無い（県軸を採らないため）。ユーザーは次の一歩に**「データ品質の下ごしらえ」を選択**。全数点検の結果**要修正67件/5,959件（1.1%）・26種**を検出（初回の「表記ゆれほぼ0」評価は誤りで、壊れ方が先頭文字の脱落なので通常の正規化をすり抜けていた）。修正後のユニーク校は約160校 | [アイデア](../raw/2026-08-12-idea-university-category-pages.md) / [チーム名の下ごしらえ](../raw/2026-08-12-university-team-name-cleanup.md) |
