...
- URL: `/secondaryschool/`（1枚）／`/secondaryschool/[prefectureId]/`（47枚）／
  `/secondaryschool/[prefectureId]/[teamId]/`（280枚）。計328枚。仕様は [secondaryschool.md](./secondaryschool.md)
...
- 内部リンク: サイドナビ「特集 > 中学」／全中の大会ハブ→`/secondaryschool/`／
  高校の学校ページ「出身中学」節→中学チームページ（81校ぶん）
