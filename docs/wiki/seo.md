...
圏外率はゼロではない。n=3 では規模が測れないので、30〜50人規模のサンプル調査が未実施の宿題。
...
| 選手ページの検索競争力（グループB） | P2a 実装済・P0 未着手 | 選手名クエリはグループA（本人資産あり・順位は実質天井）とB（本人資産なし・1〜4位を取れる）に割れる。所属歴のメタデータ化を実装。圏外率のサンプル実測が次の一手 | [2026-09-09](../raw/2026-09-09-idea-player-page-serp-competitiveness.md) |
