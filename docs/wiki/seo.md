...
### 8. 速報/結果・プレビュー記事 × 大会年度別ページ・大会ハブ ← 計画中（未実装）

- URL: `/news/[articleId]` × `/tournaments/.../[year]/.../[gender]`（年度別結果）× `/tournaments/[generation]/[tournamentId]`（ハブ）
- 重なり: 同一大会・同一年の結果（結果記事 × 年度別ページ）、および「{大会}{年} 展望・注目選手」（プレビュー記事 × ハブ）
- 制御（インテント分割）: 記事＝「確定直後の文脈・読み物／大会前の展望」、年度別ページ＝「結果記録の一次データ」、ハブ＝「歴代まとめ」。記事 title は選手名×文脈のロングテール寄せ、年度別ページは従来の「{大会}{年}結果」を維持。記事→年度別/選手/歴代ページへ内部リンクで回遊。URL は `/news` 独立ツリーに置き、大会ツリー内（`/tournaments/.../preview`）には置かない（カニバリ距離を離す）。プレビュー記事は結果確定後に同一 URL（`articleId` 共有）で結果記事へ昇格し、URL を増やさず評価を集約（canonical 統一に有利）。
- 方針: 汎用テンプレ SEO farm が押さえる「{大会}{年}結果速報」とは正面勝負しない。farm が構造的に持てない「DB 由来の文脈（歴代・通算・節目）」で差別化する。
- 状態: **未判断（計画中）**。実装後、GSC で結果記事と年度別ページのカニバリ実害を実測してから judge する。
- 詳細: [news-context-blocks.md](./news-context-blocks.md)、[raw/2026-06-21-news-auto-draft-design.md](../raw/2026-06-21-news-auto-draft-design.md)
...
- #8 速報/結果記事と大会年度別ページのカニバリ実害（実装・公開後に GSC で実測して judge）
