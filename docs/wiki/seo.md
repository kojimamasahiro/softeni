...
- 状態: **対策済／監視**（GSC で「強豪」「ランキング」系クエリの獲得と #7 との分散を確認）
...
- URL: `/secondaryschool/`（1枚）／`/secondaryschool/[prefectureId]/`（47枚）／
  `/secondaryschool/[prefectureId]/[teamId]/`（**273枚**）。計**321枚**。
  仕様は [secondaryschool.md](./secondaryschool.md)
...
- 内部リンク（2026-08-12 に相互リンクを追加）:
  ...
  | **中学チームページ「進路」→ 高校の学校ページ** | 184 | 進路レコード全件が解決（実測 184/184） |
...
- 構造化データ: 3階層すべてに `BreadcrumbList` を持たせた（高校と同水準）。
