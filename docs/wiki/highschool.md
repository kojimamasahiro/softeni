...
- **2026-09-13: 「進路」節の下に移した（案A）**。同じページに「{高校名}の選手の進路」節（h3「出身中学」＋「進学先大学」）を新設し、主な卒業生はその下に残した。説明文に掲載条件（全日本の大会でベスト8以上・STリーグ出場・国際大会出場）を明記し、網羅の「進路」と実績のハイライトという役割の違いを文言で示している（重複は277件中136件＝49%。マークは付けない）。仕様は [university.md](./university.md)「高校の学校ページの『進路』節」
...
| 学校ページの回遊強化（横リンクの新設） | **発散フェーズ（2026-08-15）**。SEO一巡後の次テーマ「回遊」の最初の着手先。目的は**PV増＝AdSense収益**。ビルド済み `out/` 4,243枚の本文内リンク実測で、`hs:school → hs:school` の横リンクが**0本**、学校ページ698枚の**68%（472枚）が本文リンク10本以下**（中央値7本の実質は定型4本＋大会年度別3本）と判明。FAQに「同じ都道府県の他校も見られますか？」があるのに答えは県ページへ1本戻すだけ（他校まで2クリック）。足せる在庫を実測した結果、**「同県の他校」が唯一全ページで成立**（med=7・0件のページ0枚・上限5校で3,293本の新規内部リンク）、**「団体戦で直接対戦した学校」は棄却**（698枚中361枚＝52%が0件、薄い472枚では73%が0件。多くの学校が個人戦のみの出場のため）。実装は既存 `getPrefectureTopSchools` の再利用で**新規URLゼロ**。未決は**絞り方**（強豪上位5校／同県全列挙／混成）。もう一方の未決だった**計測手段**は[回遊検証ランブック](./circulation-verification.md)として整備済み（2026-08-15）——主指標を同意状態に依存しない**モジュールCTR**（`internal_link_click` ÷ `page_view`）に置き、**選手結果ページ1,916枚を対照群**として季節変動を差分で打ち消す設計。最終判定はPVではなく**AdSense収益総額**。着手前に手順0（同意イベント・クリックイベント・GA4カスタムディメンション登録）の実装が必要。**※2026-09-09 追記: 同意の地域分け（[ADR-018](../adr/ADR-018-consent-by-region.md)）で日本はgrantedになったが、主指標をモジュールCTRに置く設計は変えない**（広告ブロッカーとITPによる`client_id`の不安定さは残るため）。変わるのは副指標で、セッション系の信頼度が上がる一方、前提指標だった同意率は日本で発生しなくなり事実上使えない。**ベースラインは2026-09-09より後の期間から取ること**（またぐとセッション系が段差を作る。9月末開始なら当初予定どおりで問題ない）。なお学校ページのメンバー延べ8,264名に対し選手ページへのリンクは1,237本＝15%だが、これは断線バグではなく選手ページ生成閾値（`count >= 5`、全18,543選手中10.3%）による構造的上限 | [アイデア](../raw/2026-08-15-idea-highschool-school-page-cross-links.md) |
