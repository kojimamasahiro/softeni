...
| 地区大会結果とインターハイnewsプレビューの連携 | **発散フェーズ**（2026-07-26）。news-context-blocksの④「直近大会の好成績者」(`recentAchievers`)が地区大会結果を拾える設計に近いが、`RECENT_TOURNAMENT_LIMIT=2`が9地区同時発生を想定しておらず素通しでは大半を取りこぼす。加えて団体戦も登録予定のため、個人戦専用の`recentAchievers`をそのまま使うと団体戦が漏れる（校単位の扱いを別途検討要）。拡張案（既存枠を地区大会向けに拡張／地区大会専用の新ブロック）はいずれも団体戦を含めて設計する方針 | [アイデア](../raw/2026-07-26-idea-block-tournament-news-integration.md) |
