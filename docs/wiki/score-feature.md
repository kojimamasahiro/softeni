...
「サーブ情報」の3ボタンはいずれも**トグル**です（サービスエース / ダブルフォルトは 2026-09-08 に対応）。
選択中にもう一度押す（または同じショートカットキーをもう一度押す）と未選択に戻り、
その選択で自動推定が入れた項目（勝者チーム・関与選手・ラリー数、ダブルフォルトの場合は 1stフォルトも）
を併せてクリアします。動画時刻やサーブチームなど、その選択と無関係な入力は残ります。
誤タップの取り消しに別の結果タイプを選び直す必要をなくすためです。
