...
**同点はシングルス best-1 で並び替え（2026-07-11決定・実装済み）**: 表示は種目区別をやめ
「男子／女子」の1本の板とし、**採点はダブルスのみ**だが、ポイント同点の並び順を
「その年のシングルス最高成績1大会（best-1・合算しない）の entryScore」で決める。狙いは、
同じペアが並んで区別できない問題の緩和と、シングルス好成績の可視化。**合算でなく best-1** に
するのは、撤退させた出場機会の非対称（スタック優位）を並び順にすら再流入させないため
（例: 2025男子ダブルス200点タイ→ 上松〈全日本S優勝100〉＞ 橋場〈全日本S準優勝70。学生大会優勝を
足した130は使わない〉＞ 菊山〈10〉）。1224順位のタイは `(points, singlesBest)` が完全一致した時のみ
（両者ノーシングルスの同点は正直に同順位のまま）。実装: `singlesBestBySeason`（rankingCompute.ts）。
...
- **国際大会の除外（2026-07-11決定）**: コリアカップ・平和カップひろしま（`generationId='international'`）は
  ...
  国際予選は全員国内選手のため除外しない。
  2026-09-17 に**アジア競技大会（`asian-games`）も同じ理由で追加**した（結果0件のうちに入れたので数値は動いていない。
  [raw/2026-09-17-asian-games-schedule.md](../raw/2026-09-17-asian-games-schedule.md)）。
  **2026-09-17: `asian-games` を同じ理由で追加**（結果の取り込み前に入れたので、ランキングの数値は動いていない）。
- **retired（不戦勝/途中棄権）**: 順位（placement）には反映される（勝ち上がりは事実のため）。
  勝率系の統計から除外されるのとは扱いが異なる。
- **ポイント0以下は順位表に載せない**。
- **同点は同順位（1224方式）**: `(points, singlesBest)` が一致した選手を同順位とし、次は人数分飛ばす
  （2026-07-11〜。ポイント同点でもシングルス best-1 が異なれば順位は割れる）。

## 出力・表示

- 生成物: `data/rankings/{year}-{discipline}-{gender}.json`（例: `2025-doubles-boys.json`。discipline は
  現状 doubles のみ）。行 = `{rank, playerId, playerKey, playerName, team, points, singlesBest, singlesTitle}`。
  `singlesBest` は同点並び替え用のシングルス best-1 値、`singlesTitle` は好成績の表示ラベル（優勝〜ベスト8のみ・
  無ければ null）。`playerKey` は
  **その年度の所属**を刻む（現所属で過去年度を汚染しない）。
- 表示: `/rankings` ページ（上位100位・年度/男女切替の1本の板。種目タブは廃止）と選手ページの
  「年度別ランキング推移」。ポイント列（ダブルス）に加え、同点の並び替え根拠として
  シングルス好成績バッジ（`singlesTitle`）を表示する。データ駆動のため、`disciplines` の変更や
  singles フィールド追加は再生成でページ・選手ページ双方へ自動反映（singles の順位表 JSON は
  generate-rankings の prune で削除）。
