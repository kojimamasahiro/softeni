...
- インターハイは**年度によって氏名が1文字ずつの語になる**（2023）。行ごとに欄の中の語を x 順に
  連結し、**姓と名は欄の境界（x=141・456）で分ける**。どちらの欄も均等割り付けなので
  **文字の間隔では分けられない**（`國 松 樹 人` は姓の中 18.3pt ＞ 姓と名の間 16.5pt）。
- インターハイは**年度によって収録範囲が違う**。2024〜2026 はベスト8以降の7試合だけ、
  **2023 は1回戦から全47試合**（詳細24ページ・1ページ2試合）。先に `pdftotext -layout` で範囲を見る。
- **打ち切りの対戦の本数を印字しない年度がある**（2023。2024 は `2-2` のように印字される）。
  印字が無いときだけ**ゲームから数えて `scoreA`/`scoreB` を埋める**（ADR-020 の `unfinished` は
  「途中の本数」を持つ）。印字がある年度は印字を正とし、数え直しは検算に使う。
  **`not_played`（ゲームも本数も無い）は数え直しの検査から外す**（数えるものが無い）。
- **同名の別人が2校に出ることがある**（2023 男子の佐藤直輝＝中京と羽黒。ペアが別で複数回戦とも一貫）。
  取り違えの検査を氏名だけで緩めず、取り込みは `--same-name`、prebuild は
  `check-team-match-details.mjs` の `SAME_NAME_DIFFERENT_PEOPLE`（ファイル＋氏名）で通す。
- **1回戦から載る年度では、見出しの本数が既存データ47試合ぶんの検算になる**。
  2023 男子で1件（2-0 と入っていたが PDF は 3-0）、**2023 女子ではブラケットのずれ**
  （2回戦の勝者が逆で、3回戦に上がる学校まで違っていた）が見つかった。
  詳細ページとトーナメント表の2か所で読みが一致することを確かめてから直す。
  **投入後は「学校の次の試合に同じペアが出ているか」も裏取りになる**（女子では3回戦の3ペアが
  2回戦と同じ3組で、繰り上げ先の取り違えが確かめられた）。
- **出典の誤記は、当てる値が決まらないなら印字どおり残して報告する**（補正ファイルを作らない）。
  分からないものを埋めると、後から「出典の誤記」か「こちらの判断」か区別できなくなる。
  2023 女子の `4 － ⑦`（成立しないゲーム）は印字どおり残した。本数と勝者には影響しない。
