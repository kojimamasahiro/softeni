...
- **年度で変わるのは収録範囲・氏名の組み方・打ち切りの書き方の3つ**。列の x 座標は 2022〜2026 で同じ。
  - **収録範囲**: 2024〜2026 はベスト8以降の7試合だけ、2022・2023 は1回戦から全47試合（24ページ・1ページ2試合）。
    **範囲は人の指定を鵜呑みにせず、見出し（【１回戦】(1) など）を1ページずつ棚卸しして決める**
    （2022 女子は指定より1ページ早く始まっていて、2試合落ちるところだった）。
  - **氏名**: 1つの語（2024・2026）か1文字ずつの語（2022・2023）。行ごとに欄の中の語を x 順に連結し、
...
- **対戦の塊は「－」の y 間隔で切る**（同じ対戦は約10pt・対戦の間は31pt。閾値 `RUBBER_GAP` = 25pt）。
  「打ち切り」の文字列がゲーム行を1行ぶん押し下げて 20pt の隙を作るため、15pt では1つの対戦が2つに割れる。
...
- **1回戦から載る年度では、見出しの本数が既存データ47試合ぶんの検算になる**。実際に本数の誤り1件と
  **ブラケットのずれ**（2回戦の勝者が逆で、3回戦に上がる学校まで違っていた）が見つかった。
  詳細ページとトーナメント表の2か所で読みが一致することを確かめてから直し、**直した内容は必ず報告する**。
  **投入後は「学校の次の試合に同じペアが出ているか」も裏取りになる**。
