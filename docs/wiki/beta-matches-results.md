...
> ポイント単位の記録・分析はソフトテニス固有。
> **2026-09-19 に docs 直下から移した**（内容は当時のまま。実装との照合は未実施）。
...
- 「勝敗を分けた局面候補」は、試合の勝者が取ったポイントのうち、デュース / 自分のゲームポイント（取り切った1本を除く）/ 2-2 / 相手の3連続以上を止めた場面から選ぶ（デュース最大2件、他は各1件）。**勝者が落としたゲームの中のポイントは入れない**（取っても結局そのゲームを失っており、勝敗を分けていないため）。実装は詳細ページの `decisiveMoments`

### ラリー共有リンク

1本のラリー（ポイント）を指す URL を閲覧者が共有できる。経緯は
[raw/2026-09-25-idea-rally-share-link.md](../raw/2026-09-25-idea-rally-share-link.md)。

- URL は `<試合詳細の canonical>?pointId=<point.id>`。名場面の SNS 投稿テンプレ（[rare-events.md](./rare-events.md)）・`/matches/highlights` と同じ形式
- 共有ボタンは「試合の流れ」の各ポイント・名場面カード・勝敗を分けた局面カードに置く。Web Share API があれば共有シート、なければ「共有する文面＋URL」をクリップボードへコピー
- 共有する文面は `大会名 年 回戦 / A vs B / 第Nゲーム a-b から（ラリーN本、Xのポイント）` の形。組み立ては `lib/pointShare.ts`
- **OGP は試合共通のまま**（静的 export なのでクエリで差し替えられない）
- `?pointId=` で開いたら、自動スクロール・自動再生はせず、ページ上部に「共有されたラリー」の案内と「このラリーを再生」ボタンを出す。押すとそのポイントへスクロールし、`video_start_ms`〜`video_end_ms` だけ再生して止める（`playRange`）。動画が無い試合は「記録を見る」だけ
