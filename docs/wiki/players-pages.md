...
選手まわりの公開ページの現状仕様。公開面全体は [public-pages.md](./public-pages.md)、
SEO カニバリは [seo.md](./seo.md)（#1 / #2）、データ構造は [data-model.md](./data-model.md)。
...
- **姓名の切り位置がぶれると同一人物が別 id になり、両方が `count>=5` を割って結果ページが消える**
  （`谷|明日里` / `谷明|日里`）。検出・運用は [team-player-identity.md](./team-player-identity.md)「姓名の分割ゆれ」。
...
- **既知の制約**: 効果は測れない（`alumniOf` にリッチリザルトは無く、description に足した語もほぼ本文に既出）。
  そもそも **「ページを厚くすれば順位が付く」という前提が実測で支持されていない**（[seo.md](./seo.md)）ので、
...
- **生成スクリプト**（`scripts/playerStats/`、prebuild に連結）: `generate-facts.ts` / `generate-rankings.ts` /
  ...
  [deployment.md](./deployment.md)）。成果物は `.gitignore`。
...
- **年度ランキング**はシーズンポイント制（tier × 順位係数を**その年度の上位3大会のみ合算**）。
  仕様は [ranking.md](./ranking.md)。年度別順位表の `playerKey` は**その年度の所属**を刻む（現所属で過去を汚染しない）。
...
- **ローマ字表記のみの国際大会参加者**は `resolveNumericId` で解決できないため、手動対応表
  `data/tournaments/participant-aliases.json` をフォールバックに使う（[data-import.md](./data-import.md)）。
...
SEO（詳細は [seo.md](./seo.md) #2）: title / description / 本文バッジに**通称を literal で出す**
...
2. **スタッツ**（見出し無しの平置き）: 常時表示は**主なペア（上位8名）と直近3年の成績のチップ**だけ
...
| 選手ページの検索競争力（グループB） | **P2a 実装・P0 実測済み**（2026-09-09）。圏外の原因は未特定で、次は外部要因の切り分け。title の字数予算は2026-09-22に修正済み（[seo.md](./seo.md)） | [アイデア](../raw/2026-09-09-idea-player-page-serp-competitiveness.md) |
