...
- **`players-min20.json` は廃止**（`git rm` 済み）。出場数 20 以上の 97 人ぶんの**全大会記録**を
  ...
  選手結果ページと完全重複でカニバリ側（[seo.md](./seo.md) #2）だったため一覧からは落とした。
...
- 検索はクライアント JS のみでクローラは実行しないため、検索インデックスの遅延ロード化は
  ...
  一覧からの内部リンクが 97 → 1,917 に増えたのは、[seo.md](./seo.md) #2 の「薄いページの
...
- **姓の頭文字での分割ページ（`/players/あ` 等）は作らない**（2026-08-08 決定）。姓の頭文字は
  342 種あり、うち **243 種が 5 件未満**で薄いページの量産になる。[seo.md](./seo.md) の
  「入口を1ページに集約し、重複ページ自体を作らない」方針、および `/rankings` が年度×種目×男女で
  URL を切らない判断（[public-pages.md](./public-pages.md)）と同じ理由。全件リンクは 1 ページに
...
- 所属は **最新 1 件＋「他 N」** 表示にした。表記ゆれをそのまま並べると品質の印象を落とすため。
  表記ゆれの正規化自体は名寄せ側の課題で、ここではやらない（[team-player-identity.md](./team-player-identity.md)）。
...
中止の語彙は [data-model](./data-model.md)、面ごとの見せ方は [public-pages](./public-pages.md)。
...
- **前提そのものが弱い**（2026-09-09 追記）。同日に実施した40件サンプル調査で、
  ...
  （詳細は [seo.md](./seo.md)「圏外はインデックスの問題ではなく、ページの厚さでもない」）。
...
| 読み仮名の整備 | 判断根拠が無い |
| 異体字（廣→広 等）対応 | Google が同一視していることを実測で確認（[seo.md](./seo.md)） |
...
- **リード文直後の最高成績タイル**（`src/components/PlayerMajorResults.tsx`）: h1 → リード文 →
  ...
  **1 カテゴリ = 1 タイル**で、出すのは**カテゴリ名と最高成績だけ**
  （例: 「高校 / 準優勝」）。狙いは「どのカテゴリでどこまで行ったか」が一目で分かることに絞り、
...
追加実装が要るため見送った（[open-questions.md](./open-questions.md) 参照）。
...
| アイデア | 状況・目的（1行） | 詳細 |
|---|---|---|
| 選手一覧ページ（`/players`）の再設計 | **2026-08-08 実装済み**（実機ビルドのみ未実行）。調査で「同姓同名選手の一覧」という名乗り自体が実装の誤解と判明（`differentTeams` はキャリア変遷で、実際の同姓同名 `homonyms.json` 65件と1件も重ならない）。入口として定義し直し、内部リンクを97→1,917本（19.8倍）、DOM要素数を18,207→5,798、検索インデックス334KB(gzip)の無条件fetchを廃止。姓の頭文字での分割ページは薄いページ量産（342種中243種が5件未満）のため不採用。残はGSC効果測定と読み仮名データ整備 | [アイデア](../raw/2026-08-08-idea-players-index-redesign.md) |
| 選手ページの検索競争力（グループB を攻める） | **2026-09-09 P2a 実装済**。選手名クエリはグループA（Wikipedia・公式SNS等の本人資産があり1〜10位が構造的に埋まる＝順位は実質天井）とB（本人資産なし＝1〜4位・CTR10〜25%を取れる）に割れると SERP 実測で判明。GSC の順位分布は「既に表示されているクエリ」に条件づけられるため攻め先の判断には使えない（55クエリ÷index対象1,723枚=3.2%）。所属歴を description / JSON-LD へ出す改修を実装（1,329枚）。異体字・表記ゆれ・代表所属・読み仮名は不採用。**2026-09-09 に P0（圏外率の実測・40件無作為サンプル）も実施し、圏外率72.5%／掲載時は全件3位以内（1,1,1,2,2,3,3,3,3,3,3）と判明**。選手名クエリは「3位以内か圏外か」の二値で、**「順位を上げる」は成長レバーではない**（表示されているページは既に3位以内）。レバーは圏外の約1,249枚（需要が明確なもの約732枚）。ただし圏外はインデックスの問題ではなく（小川木乃花・藤原颯介は site: で確認しインデックス済）、ページの厚さでもない（掲載/圏外で通算試合数・HTMLサイズ・内部リンク数に差が無く、むしろ圏外側に最も厚いページが並ぶ）。**原因は未特定**で、次は外部要因（被リンク・ドメイン評価）の切り分け。残は title の字数予算修正 | [アイデア](../raw/2026-09-09-idea-player-page-serp-competitiveness.md) |
| 試合結果データだけから選手の「戦評」を編み出す | 発散フェーズ、実データ一次検証済み（2026-08-06）。検索がネイキッドクエリ（選手名のみ）で来る訪問者は人物検索的な意図を持ちうるが写真・インタビューは扱わないため、試合結果（勝敗・スコア・対戦相手）だけから機械的に導出できる記述で応えたい。**接戦度（1ゲーム差以内の割合）はsplit-half信頼性検証でr≈0〜0.03とほぼ再現せず棄却**（ノイズ、ドロー依存と推測）。**勝率調整済み圧勝度（相手を完封する率から勝率で説明できる分を引いた残差）はr=0.156で有意に再現し、唯一検証を通った候補指標**。パートナー依存度は分布はあるがreliability検証未実施。姉妹アイデア「プレースタイルのプロフィール化」（score機能のポイントデータ前提）とは異なり、こちらは大会結果データのみで約8,000人規模に効く | [アイデア](../raw/2026-08-06-idea-match-result-style-commentary.md) |
