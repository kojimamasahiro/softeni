...
| 過去に負けた相手のその後の活躍を選手結果ページに出す | 発散フェーズ、実データ一次検証済み（2026-08-21）。「最近強くなった選手の過去の対戦で、負けていた場合その勝った相手が最近どう活躍したか知りたい」というユーザーの着想。対戦相手ページへの手動遷移という代替手法が既にあるため、価値は「能動的に気づかせる」部分に限定される。**新規の名寄せ面は増えない**（試合内の対戦相手特定と、相手自身の選手ページ集計は両方とも既存機能がすでに引き受けている名寄せ）。実装は既存の勲章カード基盤（`lib/nationalTitles.ts`）を読むだけの案が本命。**実データ実測**: 高校選手権個人戦ダブルス2021-2025の敗者インスタンス3,780件中、勝者が後年大学カテゴリでベスト8以上に達した件数は289件（7.6%・ユニーク敗者548人）。大学カテゴリの収録が浅い（個人戦2023年度〜3年分＋indoor2025のみ）ため、この数字は下限でインカレ2026投入後に増える見込み。未決は表示粒度・時系列条件（次カテゴリ限定か任意の後年か）・敗戦のみに絞るか | [アイデア](../raw/2026-08-21-idea-opponent-later-achievement.md) |
