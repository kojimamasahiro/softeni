...
（団体はオーダーも。混合の決勝Tは席順 `knockoutDraw` つき）。男女シングルスは予選リーグ（全47試合）の結果と、
9/23 の準々決勝の対戦カード（男子3・女子4）・席順が入った。残りは 9/23 の準々決勝〜決勝。

**会期中にやること**:

- 公式サイトの日程を見直し、変わっていれば `information/asian-games.json` の `categories[].schedule` と
  `scheduleCheckedOn` を更新する
- 9/23 のシングルスは、準々決勝の結果を入れ、準決勝・決勝の行を足して準々決勝の `next` を繋ぐ
  （id は `ms-qf2`〜`ms-qf4` / `ws-qf1`〜`ws-qf4`。今は `next: null`）。男子の準決勝1は上松（不戦勝）対 `ms-qf2` の勝者
