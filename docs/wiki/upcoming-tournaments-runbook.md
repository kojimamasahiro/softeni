...
**状態**（2026-09-23 時点）: **団体戦（男女）・混合ダブルス・女子シングルスは決勝まで入った**
（団体はオーダーも）。混合と男女シングルスは**全試合にゲームごとのポイント**がある。
男子シングルスは準決勝まで入り、**決勝（上松 対 黒坂。9/23 15:45〜）だけが未実施**（行はあり・`score` なし）。

**会期中にやること**:

- 公式サイトの日程を見直し、変わっていれば `information/asian-games.json` の `categories[].schedule` と
  `scheduleCheckedOn` を更新する
- 男子シングルス決勝（`ms-f`）が終わったら `score` と `games` を入れて作り直す
