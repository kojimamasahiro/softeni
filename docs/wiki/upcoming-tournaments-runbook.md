...
混合では A・C・D・F組の1位が不戦勝だった（4組の組の1位が1回戦から。理由は公表されていない）。
...
**状態**（2026-09-22 時点）: **団体戦（男女）と混合ダブルスは決勝まで入った**
（団体はオーダーも。混合の決勝Tは席順 `knockoutDraw` つき）。男女シングルス（25試合 / 22試合）は全組の組み合わせだけ。

**会期中にやること**:

- 公式サイトの日程を見直し、変わっていれば `information/asian-games.json` の `categories[].schedule` と
  `scheduleCheckedOn` を更新する
- 9/22〜23 のシングルスは、予選が終わったら準々決勝の組み合わせを見て `knockoutDraw` を入れる。
  **男子は準々決勝8枠のうち1枠が不戦勝**なので、席が7つしか埋まらない。
  同時に `format` の「各組1位が準々決勝」が当たっていたかを確かめる
