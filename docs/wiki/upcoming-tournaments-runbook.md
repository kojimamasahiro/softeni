...
オーダーも入れた。混合ダブルス（22試合）・男女シングルス（25試合 / 22試合）は全組の組み合わせだけ。
**決勝トーナメントの席はどの種目もまだ持っていない**（どの組が不戦勝になるかが非公開のため）。
...
- 9/22〜23 のシングルスも同様に、予選が終わったら準々決勝の組み合わせを見て `knockoutDraw` を入れる。
