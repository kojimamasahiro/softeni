...
2. **レビュー** `scripts/build-team-review-html.mjs` → `data/teams/team-merge-review.html`
   ...
   - **自動判定の誤り率は抜き取り監査で測る**（2026-09-06 追加・`npm run review:audit`）。
     ...
     **重大な制約（欠陥4・2026-09-11発見・未修正）: ブラウザの`localStorage`キャッシュが
     ...
     **回避策（恒久修正は未実装）**: レビュー画面を使う前に対象ブラウザの
     `localhost:5173`のサイトデータ（localStorage）をクリアしてから使うこと。
     恒久修正の方向性: (a) `buildOutput()`が実際にtouchしたクラスタだけを送るようにする、
     (b) `st()`の初期化をキャッシュ有無に関わらず常に台帳の`decidedBy`を信頼する形にする
     （キャッシュはgroups/canonのUI状態にだけ使う）。
