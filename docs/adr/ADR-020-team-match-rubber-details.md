...
- **`status` で状態を区別する**。STリーグ側は「必ず決着」前提なので、ここだけ広げる。
  `retired` は 2026-09-18 に追加（末尾の追記4）。
...
- **不戦勝（片側がペアを出さない）を表せない**。アジア競技大会2026 男子団体B組で、
  ネパールが第3対戦にペアを出さず、公式サイトの表示が `BYE` になった（団体の本数は 3-0 で、
  不戦勝も1勝として数える）。`completed` にすると本数を捏造することになり、
  持たないと「勝った対戦の数＝親の `scores`」が合わない。**その2試合はオーダーを持たないことにした**
  （本数だけ入れた）。案: `status` に `walkover` を足し、`scoreA` / `scoreB` は `null` にする。
  途中棄権（`retired`）は 2026-09-18 に対応済み（追記4）。
