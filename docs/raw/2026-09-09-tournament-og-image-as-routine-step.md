...
## 追記4（同日）: サイト名バーを廃止し、下に余白を作った

ユーザーから:

> og画像について改善したい。現状、タイトルバーが上部にあるが消したい。
> また、Xで表示される場合、タイトルが左下に表示されるので下部にスペースを設けたい。
> タイトルバーの高さ分スペースを追加して、その分トーナメント表などを上に上げたい

2026-07-31 の「サイト名だけを上に置き、残りの縦をすべて表に使う」という決定の見直し。
当時は **X がカードの左下にタイトルを重ねて表示する**ことを勘定に入れていなかったため、
表の一番下の行がタイトルに隠れていた。
...
| | 旧 | 新 |
|---|---|---|
| 上部 | ネイビー52px + イエロー5px のサイト名バー | なし |
| トーナメント表の上端 | 83px | **26px** |
| トーナメント表の下端 | 608px | **551px** |
| 表の縦幅（`bottom - top`） | 525px | **525px（不変）** |
| 実測の描画領域 | 0〜592px | **37〜535px** |
| 実測の下余白 | 37px | **94px**（+57 = バーの高さ） |

**表の縦幅を変えていないので、縮尺・行間・可読性は従来と同じ。** 位置だけ57px上へ寄せた。

`BRAND_BAR_H = 52 + 5` はバーを消したあとも**定数として残した**。
「バーの高さぶんを下の余白に振り替えた」という意図が、消えたバーの高さを見ないと読めないため。

サイト名は画像から消える。X はカードにドメインを出すので出所は分かる、という判断
（ユーザー指示なので採否は決定済み）。

### 全件作り直した

456枚すべてを `--apply --all` で差し替えた。追記2・3で「全件再生成はするな」と書いたが、
あれは**データが変わっていないのに描画差だけで別ファイルになる churn** の話。
今回は**描画そのものを変えた**ので、全件作り直しが正しい。この線引きを
[public-pages.md](../wiki/public-pages.md) に書き足した。

- 索引 456件のまま（キーの増減なし・456件すべて差し替え）
- `public/og/tournaments` 16MB → 17MB
- 索引の参照先456件すべてが実在・孤児0件
- dev サーバでインカレ2026 男子ダブルスとインターハイ2026 男子ダブルスが
  新しいファイル名を指していることを確認
...
| 項目 | 扱い | 理由 |
|---|---|---|
| サイト名バーを廃止した理由（X が左下にタイトルを重ねる）・57pxを下余白へ振り替えたこと・表の縦幅は不変 | wiki `public-pages.md` の OGP 節に採用。2026-07-31 の記述は消さず「変更」として上書き | レイアウトの現状は wiki が正。かつ「なぜ上にバーを置いたか」の旧理由を消すと、次に同じ提案が出たとき判断をやり直すことになる |
| `BRAND_BAR_H` を定数として残した理由 | 同節に採用 | 消えたバーの定数がコードに残っている理由は、読んだ人が必ず疑問に思う |
| 「描画そのものを変えたときは `--all` が正しい」という線引き | 同節の `--changed` の説明に採用 | 追記2・3で「全件再生成するな」と強く書いたので、例外を書かないと今回のような正当な全件作り直しまで避けられる |
| 変更前後の座標・余白の実測表 | wiki には主要な数値（57px / 525px不変 / 上37px・下94px）だけ採用 | 全部を wiki に置くと、次にマージンを触ったとき更新漏れになる |
| 456枚差し替えのサイズ影響 | 採らない（この raw に残す） | 1回きりの作業の記録 |

---

## 追記5（同日）: 右下にブランドマークだけを戻した

追記4でサイト名バーを廃止した結果、画像から出所の表示が完全に消えた。
それについて「下の余白の右端に小さくドメインを置く案もある」と伝えたところ、ユーザーから:

> タイトルが長い場合に端までくることがある。右下にアイコンだけ表示させることはできるか？

X のタイトルは左下から始まるが**長いと右端近くまで伸びる**ので、文字を置くと重なる。
マークだけなら衝突しない、という判断。

### 素材の切り出し

既定 OG カード `public/og/twitter-card-summary.png`（192×192）を使った。
インクのある行を数えると2ブロックに分かれる。

```
インクのある行ブロック: [(18, 144), (153, 170)]
  rows 18-144  cols 36-163  (w=128, h=127)   ← マーク（SP + ラケット）
  rows 153-170 cols 33-157  (w=125, h=18)    ← 「Softeni Pick」のワードマーク
```

前者だけを `BRAND_MARK_BOX = (36, 18, 164, 145)` で切る。ワードマークを外すのは、
X のタイトルと文字が二重になるため。

### 地の抜き方

元画像は**地が `(239,239,235)` の不透明 PNG**。キャンバスの地は `(250,250,248)` で
近いが同じではないので、そのまま貼ると**四角い枠が見える**。
地の色からの距離をアルファに変換して切り抜いた。
...
二値マスクにしなかったのは、38px まで縮小したときに縁がギザつくため。
距離ベースなら元のアンチエイリアスが残る。

### 配置

一辺38px、右24px（トーナメント表の左右マージンと同じ）、下20px。
占める矩形は x 1138–1176 / y 572–610。表の最下行は y≈535 で終わるので重ならない。

### 性能

アルファ生成が 16,256px の Python ループなので、456枚ぶん回すと無駄が大きい。
サイズごとに1回だけ作って使い回すキャッシュを入れた（全件生成 23秒 → 26秒。
キャッシュ無しだと数分かかる計算）。

### 検証

- 456枚すべてを再生成し、**全数**について右下 (1138,572)-(1176,610) にインクがあることを確認（0件の漏れ）
- 索引456件・PNG456枚・欠落0・孤児0
- `public/og/tournaments` は 17MB のまま

## Compile Log（追記5分）

| 項目 | 扱い | 理由 |
|---|---|---|
| 右下にマークだけを置く理由（X のタイトルが右端まで伸びる）・素材の切り出し位置・地をアルファで抜く理由・サイズと余白 | wiki `public-pages.md` の OGP 節に採用 | レイアウトの現状は wiki が正。とくに「なぜ文字ではなくマークか」「なぜ二値マスクにしないか」は、次に触る人が同じ検討をやり直す |
| ワードマークを含む元画像から2ブロックを数えて切り出したという手順 | wiki に「インクのある行を数えると2ブロックに分かれる」として採用 | 素材が差し替わったときに同じ方法で座標を出し直せる |
| アルファ生成のキャッシュ | 採らない（コード内のコメントに残す） | 実装の性能都合で、運用の判断には効かない |
| 全数チェックの方法（右下領域にインクがあるか） | 採らない（この raw に残す） | 1回きりの検証。再実行できる |
