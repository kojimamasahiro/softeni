# wiki アーカイブ: monetization.md（2026-09-18 圧縮前の全文）

2026-09-18 に [docs/wiki/monetization.md](../wiki/monetization.md) を「現在の仕様」だけに圧縮した。
圧縮前の本文（不具合の調査経緯・実測分布・日付つきの追記）をここにそのまま残す。
...
在庫の実測と閾値選定の経緯は
[raw/2026-08-25-adsense-expansion-footer-slot.md](../raw/2026-08-25-adsense-expansion-footer-slot.md)。
...
測定手順そのものは [回遊検証ランブック](../wiki/circulation-verification.md) の手順2・3をそのまま使う
...
合わせて全ページの**62%**を占めるが、[回遊検証ランブック](../wiki/circulation-verification.md) の
...
[回遊検証ランブック](../wiki/circulation-verification.md)が正で、ここは実装の所在だけを書く。
...
- **未対応**: AdSense を配信しているため、Google の EU ユーザーの同意ポリシー上、EEA/UK には
  ...
  [open-questions.md](../wiki/open-questions.md) 参照。

## score機能の新規収益化検討（2026-07-11〜）

score機能を一般ユーザーの練習試合でも使える形に広げる際の収益化案・需要調査・実データでの
パイロット分析は [score-general-availability.md](../wiki/score-general-availability.md) に分離して記載。
...
- **同じ仕組みを他競技へ広げる**（2026-09-18、発散フェーズ）: 固定費の大半が Claude の利用料なので、
  ...
  下地として wiki に適用範囲の印を付けた。**ピックルボールは2026-09-18 に調査し見送り**（需要は伸びているが
  公開結果が無く、戦績を束ねる役は DUPR が持ち、学校の枠組みが無い＝この仕組みの強みが効かない）。
  **バドミントン・ソフトボールも2026-09-18 に調査し見送り**（前者は選手名クエリに空白が無く、後者は専門メディアと
  既存のチームページが揃っている）。次に競技を選ぶときは、(1) 名前クエリの空白、(2) 一次情報がPDFだけか、
  (3) 専門メディアの薄さ、の3条件を先に当てる。詳細は raw。
