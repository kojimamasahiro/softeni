...
- ADR 化 → [ADR-019](../adr/ADR-019-team-merge-human-only.md) を作成。ADR-017 には本文を変えずに追記だけした
- エリアページの長いセル → 索引と同じ規則を広げて書き換えた（[2026-09-17-idea-backlog-index-cleanup.md](./2026-09-17-idea-backlog-index-cleanup.md) 追記2）
- 個人 skill の古い版 → アカウント側の同期だったので、claude.ai の設定から消す（同 追記3）
