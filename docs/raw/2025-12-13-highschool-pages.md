# wiki アーカイブ: highschool-pages.md（2026-09-19 統合前の全文）

2026-09-19 に docs 直下から [docs/wiki/highschool.md](../wiki/highschool.md) の
「ページ構成と読むデータ」節へ統合し、このファイルは削除した。

**この記録は 2025-12-13 に書かれたもので、統合時点の実装より古い**（当時4ページぶんしか無く、
その後 `rankings` と `tournaments` が増えている）。統合したのは現行実装と一致する部分だけで、
JSON の実例・古いページ解説はここにそのまま残す。
経緯は [2026-09-19-docs-layer-cleanup.md](./2026-09-19-docs-layer-cleanup.md)。
