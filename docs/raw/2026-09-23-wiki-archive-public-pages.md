# wiki アーカイブ: public-pages.md（2026-09-23 圧縮前の全文）

2026-09-23 に [docs/wiki/public-pages.md](../wiki/public-pages.md) が1ページの目安（12,000字）を超えたため、
...
`softeni-pick` mode: `/` / `/players/**`（[players-pages.md](../wiki/players-pages.md)）/ `/teams/**` / `/tournaments/**` /
`/highschool/**`（[highschool.md](../wiki/highschool.md)）/ `/secondaryschool/**`（[secondaryschool.md](../wiki/secondaryschool.md)）/
`/primaryschool/**`（[primaryschool.md](../wiki/primaryschool.md)）/ `/university/**`（[university.md](../wiki/university.md)）/
`/st-league/**`（[st-league.md](../wiki/st-league.md)）/ `/rankings` / `/news`・`/news/[articleId]` /
...
- **`/rankings` は1ページ**。年度・種目・男女をクライアント側で切り替える（薄いページを量産しない）。各表は上位100位。
  **タブ裏はクライアント描画なので、全年度・全種目の上位3位を静的 HTML で併載**する（[seo.md](../wiki/seo.md) #9）。
- **`/news` は大会展望（preview）専用**。結果記事は廃止（ADR-010。[news-context-blocks.md](../wiki/news-context-blocks.md)）。
...
- 表の下に「記録（最多優勝・連覇）」節と同文の FAQ（2026-09-22、`lib/championRecords.ts`）。**個人戦は選手**
  ...
  収録の無い年／中止の年をまたぐと連覇は途切れる）は [highschool.md](../wiki/highschool.md) の同節と同じ。
...
- **SEO 集中**: 高校全国大会（`getHsNationalSlugByTournamentId` が解決）と `featurePath` を持つ大会
  （現状 STリーグ）のハブは `noindex, follow` にし、誘導バナーを出す（[seo.md](../wiki/seo.md) #3）。
...
トーナメント表の組み立ては [tournament-bracket-logic.md](../wiki/tournament-bracket-logic.md)、
データの形は [tournament-data-structure.md](../wiki/tournament-data-structure.md)。
...
- **オーダーを持つ種目だけ、対戦詳細の下に「オーダーについてのよくある質問」を出す**
  （`lib/teamMatchOrderSummary.ts`。決勝のオーダーを1文にした答えと、収録試合数）。
  **画面の文面と同じものを FAQPage 構造化データに入れる**。決勝の記録が無い大会では決勝の問いごと出ない。
  description にも「オーダー」を literal で置く（[seo.md](../wiki/seo.md) #15）。
...
- **公式発表された代表名簿**（[data-model.md](../wiki/data-model.md)）がある大会に限り「日本代表に選出されている」と書ける。
...
- **対応もれの検査は `npm run check:upcoming`**（information に行が無いと**静かに出なくなる**ため）。
  終了コードは常に0で prebuild には入れない。残作業は [upcoming-tournaments-runbook.md](../wiki/upcoming-tournaments-runbook.md)。
...
- **表示回数の3割超が `?q=` 付き**なので、PV や回遊検証の分母を読むときは差し引く（[open-questions.md](../wiki/open-questions.md)）。
...
- **UI 文言**は [ux-writing.md](../wiki/ux-writing.md) が正。**収録＝母数、掲載＝表示**の使い分け（断り書きは必ず「収録」側。
...
`SportsEvent` 構造化データの項目規約（`lib/sportsEventJsonLd.ts`）は [seo.md](../wiki/seo.md)「構造化データの決めごと」。
...
全件再生成が churn になる理由）は [sns-day1-images.md](../wiki/sns-day1-images.md)「年度別結果ページの OGP 画像」。
...
| 大学カテゴリの公開ページ | **実装済み**（2026-09-13）。仕様は [university.md](../wiki/university.md) | [アイデア](../raw/2026-08-12-idea-university-category-pages.md) |
