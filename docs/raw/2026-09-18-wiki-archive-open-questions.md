# wiki アーカイブ: open-questions.md（2026-09-18 圧縮前の全文）

2026-09-18 に [docs/wiki/open-questions.md](../wiki/open-questions.md) を「未解決の問いだけ」へ圧縮した。
圧縮前の本文（解決済みの記録・調査の経緯・実測値）をここにそのまま残す。
**「解決済み（記録）」節はこのアーカイブが正**（wiki 側からは落とした）。
...
- **選手の登録名変更（改名）の対応表が未実装。**
  ...
  設計は [team-player-identity.md](../wiki/team-player-identity.md)「選手の登録名変更（改名）」、
...
- **改称した大学を1校にまとめるか。** `神戸松蔭女子学院大学` / `神戸松蔭大学`（2025年改称）、
  ...
  改称をエンティティとして扱う仕組みを作るか（[university.md](../wiki/university.md)「既知の課題」）。
...
（score機能まわりの機能アイデアは [score-general-availability.md](../wiki/score-general-availability.md) の表を参照）。
...
判断の物差しは [team-player-identity.md](../wiki/team-player-identity.md) の「姓名の分割ゆれ」節、
...
設計の詳細は [score-site-link.md](../wiki/score-site-link.md)。決定はすべて実装に入っている
（2026-09-02 に確認）:
...
- 手入力フォールバック（野良）試合に後から `siteLink` を付与して掲載大会試合へ昇格させる導線を作るか
  （現在 `siteLink` なしは1件）
- **[score-site-link.md](../wiki/score-site-link.md) 自体がドラフトの文体のまま**（「本リリースでは急がない」
...
詳細は [score-general-availability.md](../wiki/score-general-availability.md)。
...
- ~~`detected-documents.json` で `accepted` にした候補を、どの手順で `information/*.json` に反映するか~~
  ...
  [tournaments-local.md](../wiki/tournaments-local.md) の「候補検知フロー（Deprecated）」を参照）
...
wiki 反映は [players-pages.md](../wiki/players-pages.md)「選手データベース拡張」節。
...
選手結果ページのセクション階層化（[players-pages.md](../wiki/players-pages.md)「結果ページの
...
- **`normalize-team-names.mjs` の既定スコープが `highschool-japan-cup` のままである点**（2026-09-02 確認）。
  ...
  → 対応表の仕組み・登録済みエイリアス・過去の修正履歴は [data-import.md](../wiki/data-import.md) が正。
...
- **インターハイ2012 男子ダブルス・埼玉県の `春日部` がどの高校か未確定**。
  ...
  当時の出場者は吉水悠・中村匡貴。→ [team-player-identity.md](../wiki/team-player-identity.md)「alias の大会スコープ」
...
- `surface` の実在値と [data-model.md](../wiki/data-model.md) の語彙が食い違う。
...
→ [seo.md](../wiki/seo.md)「title の字数超過（大会ページ）と「インカレ 2026」の順位」、
...
（[seo.md](../wiki/seo.md) #11・2026-08-01）は `/highschool/tournaments/[tournament]` 専用で、
...
## 団体戦の対戦ごとの記録（オーダー）の残り（2026-09-18 追加）

データの持ち方・表示（対戦詳細）・集計の扱い（STリーグと同じく選手の成績集計に入れない）は決定済み
（[ADR-020](../adr/ADR-020-team-match-rubber-details.md)）。高校選抜 2022・2025 を投入済み。未決:

- **インターハイ男子団体**（記録PDFが手元に無い。女子と同じ様式なら同じコマンドで通る）と他年度。
- **ゲームごとのポイント（`games`）の見せ方**。データは 2026 女子で持っているが表示していない。
- **高校選抜の他年度**（2020・2023・2024 は出典が JSTA 以外）。
- **学校名の表記違いで結び付かない選手**（`近大高専` と `近畿大学高専` が別チーム。統合は人の判断、ADR-019）。

→ [raw/2026-09-18-idea-team-match-order.md](../raw/2026-09-18-idea-team-match-order.md)
...
[tournament-insights.md](../wiki/tournament-insights.md) が「作業手順そのものは skill にある」と
...
関連: [monetization.md](../wiki/monetization.md)「プライバシー・法務」、
...
GA4 の CSV を手で取り込む形で実装した（[public-pages.md](../wiki/public-pages.md)「トップページのよく見られているページ」）。
...
## アジア競技大会2026 の結果取り込み（方式A）の細部（2026-09-17 追加 → 2026-09-18 一部解決）

- ~~公式リザルトサイトに 9/19 の日程が無い~~ → **2026-09-18 解決**。組み合わせ公開とともに日程が更新され、
  団体の予選リーグは全組 9/18（男子団体の準々決勝も 9/18 15:00）。9/19 は試合なし
- ~~団体の予選リーグの順位表を作るため、日本の組の試合を全部持つか~~ → **2026-09-18 ユーザー判断で「日本の組は全試合」**
- **日本以外の成績（`results`）をどこまで入れるか。** 日本がいない組は持たないため、エントリーの一覧は部分的。
  日本の組のエントリーには日本以外の成績も入る想定（**Assumption**）
- **決勝トーナメントの持ち方。** 席順（`knockoutDraw`）が公式に出ていない。日本が出る試合だけを持つと、
  ブラケットの復元（ADR-015）が部分的になる。表示が壊れないかは未確認
- **方式B（全試合・外国選手も選手として持つ）をやるか**は会期後に判断する（ユーザー判断 2026-09-17）

経緯: [upcoming-tournaments-runbook.md](../wiki/upcoming-tournaments-runbook.md) S10・S11 /
[raw/2026-09-17-asian-games-schedule.md](../raw/2026-09-17-asian-games-schedule.md) /
[raw/2026-09-18-asian-games-draw.md](../raw/2026-09-18-asian-games-draw.md)
...
Open Question の側だけが更新されていなかった。→ [data-import.md](../wiki/data-import.md) /
...
→ [idea-backlog.md](../wiki/idea-backlog.md)「使い方」/
...
→ [players-pages.md](../wiki/players-pages.md)「全国大会優勝の実績表示」
...
→ [st-league.md](../wiki/st-league.md)「『メンバー』クエリの受け皿」
