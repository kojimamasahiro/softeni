# wiki アーカイブ: seo.md（2026-09-18 圧縮前の全文）

2026-09-18 に [docs/wiki/seo.md](../wiki/seo.md) を「現在の仕様」だけに圧縮した。
圧縮前の本文（日付つきの追記・実測値・検算・経緯）をここに**そのまま**残す。
圧縮の手順は [docs/prompts/slim-wiki-page.md](../prompts/slim-wiki-page.md)、経緯は
[2026-09-18-wiki-slimming.md](./2026-09-18-wiki-slimming.md)。

本文中の wiki 内リンク（`./xxx.md`）は、ここから辿れるよう `../wiki/xxx.md` に置き換えた。それ以外は原文のまま。

---

## （原題）SEO カニバリゼーション / 重複制御
...
なお **GSC で判定できるのは「既に表示されているクエリ」だけ**である。表示回数0のクエリは行として
出てこないため、圏外の語の需要は GSC からは見えない。この落とし穴と対処は
本ページ後半の「計測の原則（2026-08-28 追加）」節に分けて書いた。
...
- 詳細: [players-pages.md](../wiki/players-pages.md)「選手ページの SEO 方針」
...
- 詳細: [players-pages.md](../wiki/players-pages.md)「選手結果ページの noindex 選別」
...
- 追記（2026-07-20・全国大会優勝の通称 literal）: index に残る側のページの一意性・情報密度を上げる
  ...
  - 詳細: [players-pages.md](../wiki/players-pages.md)「主要大会の実績表示（勲章カード）と「全国大会優勝」SEO」
...
- **特集ページを持つ大会への横展開（2026-08-12）**: 同じ「ハブを noindex, follow にして特集側へ集中させる」扱いを、
  ...
  `MetaHead`）で、外すのも `featurePath` を消すだけ。詳細は [st-league.md](../wiki/st-league.md)「大会一覧との連携」
...
- 関連: [highschool.md](../wiki/highschool.md)「高校 全国大会の歴代記録ページ」、[public-pages.md](../wiki/public-pages.md)「大会ハブページ」
...
- 状態: **対策済**
- 詳細: [st-league.md](../wiki/st-league.md)
...
- 状態: **対策済**
- 詳細: [score-site-link.md](../wiki/score-site-link.md)
...
- 追記（2026-07-18）: 学校ページに「主な卒業生」セクションを追加し、「◯◯高校 ソフトテニス 出身/OB」系クエリも学校ページで受ける（詳細は [highschool.md](../wiki/highschool.md)）。卒業生の個人名クエリは従来どおり選手結果ページへリンクで集約
...
- 追記（2026-09-14・メンバー系クエリの強化）: GSC でメンバーを含む検索が確認できたのは**高校と社会人**（ユーザー確認）。学校ページのメンバーに地区大会の出場選手を足し（361ページ・1,383名）、description と FAQ に最新年と人数を出した。「◯◯高校 ソフトテニス メンバー 2026」のような年つき検索を学校ページで受けるため。title は30全角超が129ページあるので年を入れていない。詳細は [raw/2026-09-14-highschool-members-seo.md](../raw/2026-09-14-highschool-members-seo.md)
...
- 詳細: [news-context-blocks.md](../wiki/news-context-blocks.md)、[ADR-010](../adr/ADR-010-retire-result-articles-consolidate-to-hub.md)、[raw/2026-06-21-news-auto-draft-design.md](../raw/2026-06-21-news-auto-draft-design.md)
...
- 詳細: [highschool.md](../wiki/highschool.md)、[raw/2026-07-17-idea-highschool-strong-school-ranking.md](../raw/2026-07-17-idea-highschool-strong-school-ranking.md)
...
- **追記（2026-08-11・STリーグ出場チームに「メンバー」節を追加）**: 薄いページのnoindex選別とは別軸で、
  ...
  拡張（内部リンク集約と同型）。詳細は [st-league.md](../wiki/st-league.md)「『メンバー』クエリの受け皿」。
...
### 14. 小学生カテゴリ × 既存ページ（2026-09-13 追加）

- URL: `/primaryschool/`（1枚）／`/primaryschool/[prefectureId]/`（47枚）／
  `/primaryschool/[prefectureId]/[teamId]/`（**298枚**）。計**346枚**。
  仕様は [primaryschool.md](../wiki/primaryschool.md)
- **`/teams/[teamId]` との重複は0件**。#13 と同じ理由で、`/teams/` 側が生成されるのは
  `team-name-mappings.json` のキーか STリーグ出場チームだけ
- **`/secondaryschool/**` とは15件が同名**（2026-09-13 実測）。ただし**すべて同一団体**で、
  `綾瀬チャレンジ` `三国クラブ` `国分寺JSC` のように小学生の部と中学生の部の両方を持つ地域クラブ。
  URL ツリーが別（`/primaryschool/` と `/secondaryschool/`）なので同じURLにはならず、
  掲載選手の重なりも0〜6名と小さいため**内容としても別ページになっている**。
  検索意図も「{団体名} 小学生」と「{団体名} 中学」で分かれる
- **大会軸のページを作らない**ことで大会ハブとの重複も回避している。全日本小学生選手権は
  `/tournaments/junior/zennihon-primaryschool` に集約する（ADR-010）
- **薄いページの制御は掲載閾値**。出場延べ5件未満の団体は個別ページを作らない（#12 #13 と同じ値）。
  閾値なしだと672団体の56%が薄いページになる。noindex ではなく**そもそも生成しない**
- **順位づけをするページを持たない**。全日本小学生選手権は47都道府県が毎年すべて出場し
  開催県以外は全県4ペア枠という、サイト内で最も規則正しいデータだが、
  それでもポイント化はしない（#13 と同じく配点が Assumption になるため）
- 内部リンク:

  | 経路 | 本数 | 備考 |
  |---|---|---|
  | サイドナビ「カテゴリから探す > 小学生」（2026-09-14 までは「特集」）→ `/primaryschool/` | 全ページ | 並びは 小学生 → 中学 → 高校 |
  | `/primaryschool/` → 全日本小学生選手権の大会ハブ / `/secondaryschool/` | 2 | |
  | **小学生の県ページ → 中学の県ページ** | 47 | `prefectureId` は共通。中学の県ページは47県すべて存在する |
  | **小学生の団体ページ「進路」→ 中学チームページ** | 816 | レコード1063件のうち77%が解決（残りは掲載閾値未満の中学で名前だけ） |
  | **中学チームページ「出身クラブ」→ 小学生の団体ページ** | 438 | 出身クラブ延べ587件のうち75%が解決 |

  進路の順方向（小→中）と逆引き（中→小）が同じデータの表裏になっていて、
  **小 → 中 → 高の3段**が内部リンクとして繋がる。
- 構造化データ: 全ページに `BreadcrumbList`。県ページは `ItemList`、団体ページは `SportsTeam`
- **未対応**: 同名同県の15団体のうち3件（`田辺STC` `向原JST` `岩見沢少年団`）は、
  進路レコードに自分の中学部が含まれないため小学ページと中学ページが相互に繋がっていない。
  残り12件は進路節が自然に繋いでいる
...
- URL: `/secondaryschool/`（1枚）／`/secondaryschool/pathways/`（1枚）／
  ...
  計**341枚**。仕様は [secondaryschool.md](../wiki/secondaryschool.md)
...
- 内部リンク（2026-08-12 に相互リンクを追加）:
  ...
  | 経路 | 本数 | 備考 |
  |---|---|---|
  | サイドナビ「カテゴリから探す > 中学生」（2026-09-14 までは「特集 > 中学」）→ `/secondaryschool/` | 全ページ | |
...
- 構造化データ: 全ページに `BreadcrumbList` を持たせた（高校と同水準）。
  ...
  ページ側で個別に書かない（[public-pages.md #パンくずの構造化データ](../wiki/public-pages.md#パンくずの構造化データbreadcrumblist)）
...
重複制御とは別軸の問題。**大会の正式名称でページを作った結果、検索する側が使う語が
ページ上に1回も存在せず、そのクエリで圏外になる**という失敗の型。全中で実際に起きた。

- 症状: `/tournaments/junior/secondaryschool-championship/`（全中ハブ）は
  「全国中学校体育大会」としか名乗っておらず、**「全中」も「全国中学校ソフトテニス大会」も
  literal で0回**。「ソフトテニス 全中 2026 結果」の SERP 上位30件に当サイトは出ない（2026-08-28 実測）。
  上位の競合は全員、title にどちらかの literal を持つ。
- 対比として、**兄弟のブロック大会は `◯◯中学校ソフトテニス大会` と名乗っており勝っている**
  （近畿は掲載順位3.16位・CTR 32%）。全中だけがこの型から外れている。
  正式名称は確かに「全国中学校体育大会」だが、検索する側は競技名で探す。
- 制御: **正式名称を置き換えるのではなく併記する**。title / h1 / description に
  検索語の literal を1〜2回入れる。#3 の「ハイジャパ」で採ったのと同じ手段
  （専用ページを作らず既存ハブに literal を集約する）。
- **狙い先は「歴代」に寄せる**。全中の速報クエリは farm（zutto-sports・iezo・softtennishitting）と
  公式・YouTube ライブ配信が占めており、#8/#11 の「速報では farm と正面勝負しない」方針が
  そのまま当てはまる。一方 Google の関連検索には「ソフトテニス 全中 歴代」が出ており、
  歴代は farm が構造的に持てず当サイトが「インターハイ 歴代」で勝っている土俵。
  **全中ハブはまさに歴代ページなのに全中と名乗っていない**のが現状。
- 実装上の未解決: 汎用ハブ `/tournaments/[generation]/[tournamentId]/` は
  `data/tournaments/index.json` の `label` しか読まず、`lib/nationalTitles.ts` の
  `aliases`（全中は登録済み・選手ページの勲章カード用）を参照していない。
  高校ハブだけが `lib/highschoolNationalTournamentMeta.ts` 経由で aliases を通している。
  汎用ハブに検索用別名の口を作るか個別対応かは**未判断**。
- **順位とCTRを取り違えないこと**: 全中ハブは「全国中学校ソフトテニス大会 2026 結果」で
  30表示・0クリック・8.77位だが、その順位の期待CTRは1〜2%（30表示で0.5クリック相当）なので
  **CTRは異常ではない。課題は順位**。スニペット改善に向かうと的を外す。
- 状態: **未着手**（原因特定まで完了）。次の会期は2027年8月で、
...
- ~~`zennihon-university-ouza`（王座決定戦）: SERP上で大学対抗戦も「インカレ」と呼ばれている
  ...
  が入っているため `searchLabel` は不要）~~ → **2026-09-07 撤回**（下記）
...
#### 王座決定戦の「インカレ」alias は撤回（2026-09-07）

`zennihon-university-ouza`（全日本大学ソフトテニス王座決定戦）の
`searchAliases: ["インカレ"]` を**削除した**。ユーザー指摘のとおり、王座決定戦は
インカレ**ではない**（インカレ＝全日本学生選手権大会）。alias を入れると
`buildTournamentSearchNames` の `headingName` が
「インカレ（全日本大学ソフトテニス王座決定戦）」となり、ハブ・年度別結果ページの
title / h1 が**別大会の名前で王座決定戦を名乗る**状態になっていた。

2026-09-03 の判断根拠は「SERP上で大学対抗戦も『インカレ』と呼ばれている」だったが、
SERP に混同した表記が存在することは**当サイトが同じ混同を再生産してよい理由にならない**。
検索語の literal を持たせる施策は、**その語がそのページの大会を指しているときにだけ**適用する。
これが `searchAliases` の適用条件として今回はっきりした境界線。

`searchNote` は残し、文面を「全日本学生選手権大会（インカレ）**とは別の大会で**、同時期に
開催される大学対抗戦です。」に改めた（混同を解くテキストとしてはむしろ有用なため）。
`zennihon-university` 側の alias は正しいので変更しない。


## title の字数超過で Google に書き換えられている（選手ページ）（2026-09-09 追加・未修正）

2026-07-20「全国大会優勝の通称を title に literal で出す」施策の事後検証。
Open Question に挙げていた「title 変更後の SERP タイトルが書き換えられずに出るか」の答えは
**書き換えられていた**。
...
対して廣瀬礼衣（26全角）は `<title>` がそのまま出ている。Google の表示枠は概ね28〜32全角。

ビルド済み `out/` の実測（index対象の選手結果ページ 1,723枚）:

- title 表示幅の中央値 26.5全角
- **30全角超が245枚（14.2%）**、32超が187枚（10.9%）、40超が75枚（4.4%）
- 超過しているのはほぼ全て上記施策の対象者。つまり**最も押し上げたいページだけが
  title を捨てられている**

原因は `lib/nationalTitles.ts` の `TWO_LABEL_MAX = 26` が**実績フレーズだけ**を予算にしており、
`{選手名}（{所属}）` の接頭辞と `｜試合結果・戦績 | ソフトテニス`（14全角）の接尾辞を
数えていないこと。修正するなら**完成形の表示幅**で予算を持つ形にする。

状態: **未修正**（2026-09-09 時点。原因特定のみ）。

## title の字数超過（大会ページ）と「インカレ 2026」の順位（2026-09-09 追加・対策済）

同じ「表示枠 28〜32 全角」の問題が**大会ページ側にもあった**。原因は別で、
選手ページが実績フレーズの予算ミスなのに対し、大会ページは**名前そのものが長い**こと。

きっかけは「ソフトテニス インカレ 2026」が 20位台という指摘。
[2026-09-03 の alias 対策](#インカレ全日本学生選手権大会にも同じ対策を適用2026-09-03-追加)で
圏外からは脱していたので、次の律速を SERP 実測で切り分けた。

### 症状（2026-09-09 実測）

- ヒットしていたのは `/zennihon-university/2026/singles/none/girls/`（**女子シングルス**）で
  概ね28位。6種目ページが**ほぼ同一の title**（差は男女と種目だけ）で並んでおり、
  Google 側に「インカレ2026」の受け皿を決める材料が無かった
- ビルド済み `out/` の全数実測: 汎用ハブ69枚の**99%**・年度別結果476枚の**100%**が
  30全角超（中央値はハブ 38.5全角・年度別 42.0全角）。年度別ページでは
  `2026` の開始位置が **22.5全角目**で、「結果」「トーナメント表」は SERP に出ていなかった
- 冒頭を使い切っていたのは `headingName`（＝`略称（正式名称）`。インカレで22全角）
...
| 2026年度別 6枚 | 11〜69枚 | あり |

**年を名乗るページに評価が集まっておらず、評価が集まるページが年を名乗っていない。**
[#3 追記3（2026-08-06）](#3-大会ハブ年なし--高校全国大会-歴代記録--未判断の主リスク)で
インターハイ vs 全日本シングルスの順位差の主因と特定したのと同じ構造。

あわせて、高校ハブに 2026-08-01 に入れた**開催中モードは汎用ハブに移植されていない**
（#11）。インカレ・全中・全日本選手権など汎用ルートの大会は全部この恩恵を受けていない。

### 対策（2026-09-09 実施）

- `lib/tournamentSearchNames.ts` に **`titleLeadName`**（略称 → 検索名 → label）を追加。
  title の先頭はこれにし、正式名称は h1 / description / JSON-LD の `alternateName` 側で
  literal を確保する。高校ハブで先に採った手法（#3 追記4）を汎用ルートへ横展開したもの
- **汎用ハブ**: `{titleLeadName}{最新収録年} 結果・歴代優勝者`。
  年は**前年以降のときだけ**出す（収録が古い大会で年を出すと「そこで止まっている」表示に
  なるだけ）。判定用のビルド年は `getStaticProps` で確定させる
  （コンポーネント側で `new Date()` を呼ぶと年またぎで hydration mismatch）
- **年度別結果ページ**: `{titleLeadName}{年} {種目} {結果・組み合わせ}`。
  **「組み合わせ」は実需クエリなのに literal で0回だった**（関連検索に2回出る。
  「トーナメント表」は同義語であって検索語ではない）。当サイトは 585ペア＋84選手の
  完全なドローを構造化データで持ち、競合は PDF リンクと X 投稿しか持っていないので、
  ここは #8 / #11 の「速報では farm と正面勝負しない」方針とは別に**正面から取れる**。
...
- **展望/結果記事**（`lib/newsArticle/`）: `tournamentMetaOf` が `searchLabel`/`searchAliases` を
  ...
  インカレの展望記事は 6,081字のユニークな本文を持ちながら
  「インカレ」が**ページ全体で0回**だった

検算（dev サーバ実測）: インカレ ハブ 47.5→**24.0全角**、2026男子ダブルス 52.0→**27.5全角**、
展望記事 36.0→**27.0全角**。回帰側は「全日本シングルス 歴代」#3 のハブが 33.0全角で
「歴代」は 18.5全角目（従来 16.5全角目）と枠内に残り、全数シミュレーションでも中央値は
ハブ 38.5→32.0全角・年度別 42.0→37.5全角。

状態: **対策済／監視**（GSC で「インカレ 2026」系クエリの順位・表示回数を確認）。

見送り・未着手:

- **汎用ハブへの開催中モード移植**（#11 の横展開）。インカレの会期は当日終了で、
  優勝確定と同時に自動で歴代モードへ戻る設計のため2026年分には間に合わない。
  次に会期を迎える汎用ルートの大会（全日本選手権・全中・全日本社会人など）の前が期限
- **高校大会の news 記事**は今回の恩恵を受けない。高校の略称マスタは
  `lib/highschoolNationalTournamentMeta.ts` にあり `index.json` 側が空のため、
  `/news/highschool-championship-2026/` の title に「インターハイ」literal が無いまま。
  略称マスタが3つに分かれている問題（`tournamentSearchNames.ts` 冒頭）がそのまま出ている
- **内部リンクのアンカーテキスト**。ハブへの内部リンク869本のうち867本が
  「全日本学生選手権大会」で「インカレ」は0本。`out/` 全体でも
  「全日本学生選手権大会」を含む925枚のうち**808枚が「インカレ」を1度も含まない**。
  展開するなら #2 追記が警戒した「同じフレーズの機械的な反復」を避ける設計が要る

経緯: [raw/2026-09-09-incare-2026-serp-position-and-title-budget.md](../raw/2026-09-09-incare-2026-serp-position-and-title-budget.md)

## 計測の原則（2026-08-28 追加）

### GSC の不在は需要の不在ではない

**GSC は表示回数0のクエリを行として出さない。**圏外の語は、需要の大小にかかわらず
エクスポートから消える。したがって「GSC に出てこない＝検索されていない」は**誤り**である。
2026-08-28 に実際にこの誤読をやり、SERP を直接見て覆した（全中は需要があり、当サイトが
圏外だっただけだった）。

- 例外: **観測できた exact string については**、掲載順位がページ1圏内なら
  表示回数≒検索回数と読んでよい（出ている以上、出そこねはほぼ無い）。
  誤りになるのは、観測できた数語から**そのテーマ全体へ一般化する**とき。
  クエリ文字列が1文字でも違えば GSC 上は別の行であり、そちらは圏外かもしれない。
- **エクスポートの絞り込みを確認する**。「結果」を含むクエリで絞ったデータからは、
  「ソフトテニス 全中」のような語が構造的に見えない。絞り込み条件を書き添えずに
  数値だけ残すと、後から読んだ人が全体像と誤認する。
- **圏外かどうかの判定手段は SERP を直接見ること**。GSC は「出たあと」しか観測できない。

### GSC の順位分布は「攻めるべきか」の判断に使えない（2026-09-09 追加）

同じ落とし穴の別の顔。選手名クエリ55件の掲載順位分布が「96.4% が10位以内」でも、
**それは「既に表示されているクエリ」に条件づけられた分布**であり、圏外のページは
行として存在しないので分布に入りようがない。「順位は既に良いから順位以外を攻める」という
読みは、この母集団からは導けない。

母数の目安を必ず併記すること: 55クエリ ÷ index対象の選手ページ 1,723枚 = **3.2%**。
残り 1,668枚が「需要が無い」のか「圏外」なのかは GSC からは判別できない。
判別手段は SERP を直接見ること（サンプル調査）。

実際に GSC に出ていない選手3人を SERP で確認したところ、庭田咲月2位・廣瀬礼衣1位に対し
**木原恵菜は圏外**だった（引退報道・U-20ナショナル・実業団所属で需要は明確）。

**2026-09-09、40件の無作為サンプルで規模を測った（下記「選手名クエリは3位以内か圏外か」節）。**

### 選手名クエリは3位以内か圏外かの二値（2026-09-09 追加・実測）

index 対象の選手結果ページ 1,723枚から無作為に 40件を抽出し、各氏名を Google で検索した
（非ログイン・`num=20&hl=ja`）。
...
**掲載したときの順位は 1,1,1,2,2,3,3,3,3,3,3 で全件が3位以内**。中間が1件も無い。
ソフトテニス需要が明確な層（n=24）に絞っても圏外は 70.8%。

1,723枚への外挿: 掲載 約474枚 / 圏外 約1,249枚（需要が明確なもの 約732枚）。
一方 GSC に表示が出ているのは55クエリ（3.2%）で、**順位が付いているページですら大半は
GSC に行として現れない**。

**これは施策の優先順位を変える。**表示されているページは既に3位以内にいるので
「順位を上げる」は成長レバーではない。レバーは 72.5% の圏外側で、規模も桁が違う。
ユーザー提示の GSC 分布（3位以内27.3%）と本サンプルの掲載率（27.5%）が一致するのは、
**同じ母集団を別の窓から見ている**ため。

### 圏外はインデックスの問題ではなく、ページの厚さでもない（2026-09-09 追加）

需要が明確な圏外選手を `site:softeni-pick.com {氏名}` で確認すると、
小川木乃花（ダンロップ・通算77試合）と藤原颯介（FSTA・全国小学生優勝）は
**自分の結果ページがインデックスされているのに氏名クエリでは圏外**だった。
2026-06 の noindex 選別が対象にした「クロール済み - インデックス未登録」とは別の、
**ランキングの問題**である。

さらにサンプル40枚の生成 HTML を計測したところ、掲載側と圏外側で差が無かった。

| 指標（中央値） | 掲載 n=11 | 圏外 n=29 |
|---|---|---|
| ページ上の通算試合数 | 20 | 19 |
| HTML バイト数 | 84,710 | 75,699 |
| 内部リンク数 | 54 | 50 |
| title に「優勝」を含む | 1件 | 2件 |

しかも圏外側に最も厚いページが並ぶ（吉木理彩 通算95試合・内部リンク114、
小川木乃花 77試合・103、原田美結 51試合・89）。

**「ページを厚くすれば順位が付く」という前提は、この母集団では支持されない。**
選手ページの情報量を増やす施策（所属歴のメタデータ化・戦評アイデア等）を立てるときは、
この否定的結果を踏まえること。今回測った範囲では掲載と圏外を分ける要因を特定できておらず、
次に切り分けるべきはページ内部ではなく外部要因（被リンク・ドメイン評価・クエリごとの競合の質）。

経緯: [raw/2026-09-09](../raw/2026-09-09-idea-player-page-serp-competitiveness.md)

経緯: [raw/2026-09-09](../raw/2026-09-09-idea-player-page-serp-competitiveness.md)

### 選手名クエリは2つの母集団に割れる（2026-09-09 追加）

同じ「選手名クエリ」でも、SERP の競争環境がまったく違う。施策の ROI を分けて考えること。

- **グループA（Wikipedia・公式SNS・スポンサー・JOC などの本人資産を持つ）**: 1〜10位が
  本人所有プロパティで構造的に埋まる。船水颯人の11位は「公式・SNS を除いた最初の枠」で実質天井。
  さらに関連検索が 年収 / 引退理由 / 現在 / 結婚 / ラケット / 兄弟 と**伝記的意図**に偏っており、
  当サイトは写真・インタビューを扱わない方針なので、順位を上げても CTR は伸びにくい。
  この層で狙うなら選手名単独ではなく複合クエリ（「{選手名} 2026」「{選手名} 成績」）。
- **グループB（本人資産なし）**: 当サイトがその人物についてウェブ上で最も構造化された
  情報源になれる。実際 1〜4位・CTR 10〜25% を取れている（廣瀬礼衣1.30位・CTR 24.66%）。

判別の目安: curated プロフィール23選手はおおむねグループA。GSC の代表例5人でも、
下位3人が全員 curated、上位2人が非 curated だった。

### 検索側の異体字は Google が同一視する（2026-09-09 追加・棄却）

氏名の旧字体（廣→広、澤→沢、髙→高 等）が検索語と食い違う懸念は**実測で否定**した。
「広瀬礼衣」（常用字体）で検索して当サイトの「廣瀬礼衣」ページが1位を取り、SERP に
「含まれない: 広瀬 / 必須にする: 広瀬」と明示された＝ Google 側で同一視されている。

結果ページを持つ 1,959人中 140人（7.1%）が氏名に旧字体を含むが、**対策は不要**。
同様に所属名の表記ゆれも index対象 1,723枚中2枚のみで、施策の対象にならない。
再調査しないよう、棄却の事実をここに残す。

### Google Trends は競技横断の略称に使えない

「全中」は全国中学校体育大会の略称で**全競技共通**（バレー・バスケ・陸上・卓球…）。
しかも各競技の全中は同じ8月中下旬に開催されるため、**山の立つ時期まで重なる**。
Trends の「全中」単独の山からソフトテニスの需要は読めず、過大評価になる。
競技名と組み合わせた語で見るか、SERP・GSC で裏を取ること。

- 逆に Trends にしかできない仕事もある。GSC が表示回数0を映さない以上、
  **完全に圏外の語の需要を知る手段は Trends か SERP しかない**。役割分担として使う。
- ただし Trends の「急上昇」は**相対的な伸び**であって絶対量ではない。
  2→20 でも急上昇と表示される。需要の大きさの根拠には使えない。

## 発展候補アイデア一覧（Idea Backlog）

| アイデア | 状況 | 一言 | raw |
|---|---|---|---|
| 全中の検索語と大会名表記の乖離を埋める | 原因特定済・実装未着手 | 全中ハブが「全中」「全国中学校ソフトテニス大会」を literal で0回しか持たず、主要クエリで SERP 圏外。狙い先は速報ではなく「全中 歴代」。2027年7月上旬が実装期限 | [2026-08-28](../raw/2026-08-28-idea-zenchu-abbreviation-seo.md) |
| 中学ブロック大会の横展開（近畿以外） | 未調査 | GSC のクリック87%を近畿の中学ブロック大会が稼ぐ一方、九州(9.75位)・東北(8.5位)・関東(11位)は沈んでいる。理由未調査 | 同上 |
| 選手ページの検索競争力（グループB） | P2a 実装済・**P0 実施済** | 選手名クエリはグループA（本人資産あり・順位は実質天井）とB（本人資産なし）に割れる。所属歴のメタデータ化を実装。**40件サンプルで圏外率72.5%・掲載時は全件3位以内と判明し、「順位を上げる」は成長レバーではないと確定**。圏外はインデックスの問題でもページの厚さでもなく、原因は未特定 | [2026-09-09](../raw/2026-09-09-idea-player-page-serp-competitiveness.md) |
| title の字数予算を完成形の表示幅で持つ | 原因特定済・実装未着手 | 245枚（14.2%）が30全角超で SERP タイトルを書き換えられている。`TWO_LABEL_MAX` が接頭辞・接尾辞を数えていない | 同上 |
...
1. `next-sitemap.config.js` — 列挙・`exclude`・`lastmod`・`additionalPaths`。
   **出力先は `outDir: 'out'` を明示**（既定の `public/` のままだと配信 sitemap が
   1ビルド古くなる。理由は [deployment.md](../wiki/deployment.md#sitemap-の出力先2026-08-05-修正)）
...
- **#3 / #7 / #10 の GSC 事後検証は [高校SEO M4検証ランブック](../wiki/highschool-seo-m4-verification.md) に手順化済み（2026年8月中旬に実行）**
...
- **#3 の「SERP タイトルが書き換えられずに出るか」は 2026-09-09 に決着（書き換えられていた）。
  詳細は下記「title の字数超過」節。順位の事後検証だけが残っている**
...
- #11（2026-08-01 追記）会期中に更新した3種のページ（ハブ／都道府県94枚／学校243枚）が
  ...
  計測は [高校SEO M4検証ランブック](../wiki/highschool-seo-m4-verification.md) と同じタイミング（8月中旬）
