...
### 追記: 主要タイトル表の年を2022年以降に絞った（2026-09-06・ユーザー指示）

「主要タイトルの年は2022年からにしたい」。`MAJOR_TITLE_START_YEAR = 2022` を入れた。

2022 は**主要4大会がそろって収録されている最初の年**で、それより前は同じ `ー` が
3つの別の意味になっていた（ミックスは2020年創設でそもそも存在しない／インドアは収録が
2022年（第68回）からで未収録／天皇賜杯・シングルスだけ2016年から埋まっている）。

副作用として、**この表からは 2020・2021 の「中止」列も消える**（中止の表示は大会一覧・
大会ハブ・歴代優勝者表には残る）。また 2021年以前だけに出場歴がある選手（実測1,562人）は
この表が全て `ー` になる。2016〜2021 の成績自体は年度別の大会結果カードと勲章カード
（`lib/nationalTitles.ts` 系・24大会）に従来どおり出る。

**表の直下に断りを追加した**: 「※ 2022年以降を表示しています。それ以前の成績は年度別の
大会結果に掲載しています。」——ページ上部の title / description は「全日本選手権優勝5回」の
ように2021年以前も数えており（船水颯人の実例）、断りが無いと同じページ内で食い違って見える。
