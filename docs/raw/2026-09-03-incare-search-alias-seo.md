...
## 追記（2026-09-07）: `zennihon-university-ouza` の alias は撤回

ユーザー指摘「（王座決定戦が）インカレとなっているがインカレではない」を受けて、
`zennihon-university-ouza` の `searchAliases: ["インカレ"]` を削除した。

当時「SERP上で大学対抗戦も『インカレ』と呼ばれていることを確認できた」ことを根拠に
alias を付けたが、これは判断ミス。alias を付けると `headingName` が
「インカレ（全日本大学ソフトテニス王座決定戦）」になり、ハブ・年度別結果ページの
title / h1 で**王座決定戦が別大会の名前を名乗る**。SERP に混同表記が存在することは、
当サイトが同じ混同を再生産してよい理由にはならない。

得られた境界線: `searchAliases` は「**その語がそのページの大会そのものを指す**」ときだけ
使う。「その語で検索する人がこのページも見たがるかもしれない」は alias の根拠にならない
（それは関連リンクや `searchNote` の仕事）。

`searchNote` は残し、文面を「全日本学生選手権大会（インカレ）**とは別の大会で**、同時期に
開催される大学対抗戦です。」に改めた。混同を解くテキストとしてはむしろ有用なため。
`zennihon-university`（インカレ本体）の alias は正しいので変更していない。

wiki 側は [seo.md](../wiki/seo.md) の当該節に取り消し線＋「王座決定戦の「インカレ」alias は
撤回（2026-09-07）」小節として反映済み。
