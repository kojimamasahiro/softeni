...
- **残タスク(オーナー環境)**: `npm run build` → デプロイ → 表示・301疎通確認 → GSC を2週間監視(M3 完了条件)
- 次: M4(ページ構造の統一)。M3 の監視期間と並行可(URL 変更を伴わないため)
...
decisions.md を参照。現時点の登録: D-001〜D-010(D-010 = Phase 4〜8 成果物の一括承認)
...
| O-003 | /tournaments/major・/tournaments/local・/highschool/tournaments へのナビ導線の実配置確認 | 2026-07-04 | 未決 |
| O-004 | 各ページの実表示確認(スクリーンショットベース) | 2026-07-04 | 未決 |
| O-005 | adinsight-site/ ディレクトリの位置づけ | 2026-07-04 | 未決 |
| O-006 | Supabase 側スキーマ詳細の要否 | 2026-07-04 | 未決 |
| O-007 | 高校チーム(文字列 id)とチームマスタ(連番 id)の統合要否 | 2026-07-04 | 未決 |
| O-008 | 選手の所属変遷(進学・移籍)のモデル上の扱い | 2026-07-04 | 未決 |
| O-009 | 「試合記録」のナビ露出可否 | 2026-07-04 | 解消(D-011: 出さない) |
| O-010 | /teams 一覧の検索仕様 | 2026-07-04 | 解消(D-014: count>=2、リンクは当面STリーグチームのみ) |
| O-011 | /rankings のページタイプ扱い(将来のデータボード型独立) | 2026-07-04 | 未決(該当ページ3つ以上で再検討) |
| O-012 | ダークモード方針 | 2026-07-04 | 解消(D-012: 将来対応・dark: は置換) |
| O-013 | トークン実装方式(tailwind.config or CSS 変数) | 2026-07-04 | 未決(M5 着手前に判断) |
...
3. 残る未決事項の推奨タイミング: O-010 → M2 着手前、O-013 → M5 着手前、O-003〜O-008 → 関連する移行単位で随時、O-011 → 条件成立時
