...
| TeamLink | (1) チームページ(STリーグ集計あり)→ `/teams/[teamId]`、(2) 高校チーム → 学校ページ `/highschool/[gender]/[prefectureId]/[teamId]`、(3) いずれもなし → 名前のみ。**(2) は id 統合(O-007)を待たず (校名, 都道府県) 照合で解決する**(2026-08-11、D-025)。高校は男女で URL が分かれるため、性別を選んでいない文脈では男子・女子を併記する |
