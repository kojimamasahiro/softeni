...
| インターハイ（公式記録） | 年度で違う。2024〜2026 は**ベスト8以降だけ**、**2022・2023 は男女とも1回戦から**。ゲームごとのポイントつき | `highschool_championship_team_matches.py` |
...
- **「打ち切り」の印字をあてにしない**。インターハイは印字がある対戦と、`3-3` のまま印字が無い対戦がある。
  勝者は**本数の丸数字の有無**で決める。さらに**「打ち切り」の文字列は同じ対戦のゲーム行を1行ぶん押し下げる**。
  塊を「－」の y 間隔（同じ対戦10pt・対戦の間31pt）で切っているので、閾値が狭いと1つの対戦が2つに割れる
  （2024 男子で `RUBBER_GAP` を 15→25 に上げた）。「塊 N件 / 試合 M件」で止まったらまずここを疑う。
