...
import { buildYouTubeWatchUrl, loadVideoSessionsForMatch, parseYouTubeVideoId } from '@/lib/videoReview';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      return sendSupabaseError(res, error as Error, 'Failed to load video sessions.');
    }
  }
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      const youtubeVideoId = body.source_type === 'youtube' && body.source_url ? parseYouTubeVideoId(body.source_url) : null;
      ...
      const { data: session, error } = await (supabase as any)
        ...
        .insert({
          ...
          source_url: body.source_type === 'youtube' ? (buildYouTubeWatchUrl(body.source_url ?? '') ?? body.source_url) : null,
      ...
      return sendSupabaseError(res, error as Error, 'Failed to create video session.');
    }
  }
