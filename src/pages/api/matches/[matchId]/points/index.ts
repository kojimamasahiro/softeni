...
import { getServerSupabase, recomputeGameScore, renumberPoints, sendMethodNotAllowed, sendSupabaseError } from '@/lib/matchesApi';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      if (!body.game_id || typeof body.point_number !== 'number') {
        return res.status(400).json({ error: 'game_id and point_number are required.' });
      }

      if (body.video_end_ms !== null && body.video_end_ms !== undefined && (body.video_start_ms === null || body.video_start_ms === undefined)) {
      ...
      const { data: point, error } = await (supabase as any).from('points').insert(body).select('*').single();
  ...
  if (req.method === 'PUT') {
    ...
    try {
      ...
      const { data: existingPoint, error: existingPointError } = await (supabase as any).from('points').select('*').eq('id', point_id).single();

      if (existingPointError) {
        throw existingPointError;
      }

      const nextVideoStartMs = 'video_start_ms' in updates ? updates.video_start_ms : existingPoint.video_start_ms;
      const nextVideoEndMs = 'video_end_ms' in updates ? updates.video_end_ms : existingPoint.video_end_ms;

      if (nextVideoEndMs !== null && nextVideoEndMs !== undefined && (nextVideoStartMs === null || nextVideoStartMs === undefined)) {
        return res.status(400).json({
          error: 'video_start_ms is required when video_end_ms is provided.',
        });
      }
      ...
      const { data: point, error } = await (supabase as any).from('points').update(updates).eq('id', point_id).select('*').single();

      if (error) {
        throw error;
      }

      const { updatedGame } = await recomputeGameScore(supabase, existingPoint.game_id);
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      ...
      const { error } = await (supabase as any).from('points').delete().eq('id', point_id);
      if (error) {
        throw error;
      }
