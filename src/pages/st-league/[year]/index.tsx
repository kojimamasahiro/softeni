...
export default function STLeagueYearHub({
  year,
  meta,
  divisionNames,
  champions,
  ...
  const dateRange = meta?.period
    ...
    : '';
  const championText = [
    champions.boys && `男子${champions.boys}`,
    champions.girls && `女子${champions.girls}`,
  ]
    .filter(Boolean)
    .join('、');
      ...
      <PageLayout maxWidth="4xl" className="space-y-8">
        ...
        <header>
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-300 leading-relaxed">
            ...
            の開催概要・会場・優勝チームをまとめたページです。男女・リーグ別の
            詳しい試合結果や順位表、出場チーム・選手データは下記の各ページから確認できます。
          </p>
        </header>
          ...
          <section>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {GENDERS.map(({ key, label, ring }) => (
                <div
                  key={key}
                  className={`bg-white dark:bg-gray-800 rounded-xl p-5 border ${ring} shadow-sm`}
                >
