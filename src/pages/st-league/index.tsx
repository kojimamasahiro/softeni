...
import { getDivisions, getStLeagueYears, loadLeagueMeta } from '@/utils/st-league';
...
export default function STLeagueHub({ editions, overview, divisionOverview }: Props) {
    ...
    <>
      ...
      <PageLayout maxWidth="6xl">
        ...
        <section className="max-w-3xl mx-auto mb-10 px-4">
          <h1 className="text-2xl font-bold mb-4">STリーグ 結果・順位表・出場チーム</h1>
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2">
            ...
            <Link href="/st-league/champions" className="inline-flex items-center text-link font-semibold hover:underline">
              歴代優勝チーム・記録を見る
              <svg className="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
              </svg>
            </Link>
            <Link href="/st-league/teams" className="inline-flex items-center text-link font-semibold hover:underline">
              掲載チーム一覧を見る
              <svg className="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
              </svg>
        ...
        {divisionOverview.length > 0 && (
          <section className="max-w-4xl mx-auto mb-12 px-4">
            ...
            <p className="text-xs text-gray-400 mt-3">※ 上位・下位リーグ間ではプレーオフ（入替戦）により昇格・降格が行われます。</p>
          </section>
        )}
        ...
        <div className="max-w-4xl mx-auto">
          <section className="mb-8 px-4">
            ...
            <div className="space-y-10">
                ...
                <div key={ed.year} className="relative bg-surface rounded-2xl p-8 shadow-sm overflow-hidden">
                  <div className="relative z-10">
                    ...
                    <h3 className="mt-2 text-xl font-bold">
                      <Link href={`/st-league/${ed.year}`} className="hover:text-blue-600 hover:underline transition-colors">
                        {ed.title}
                      </Link>
                    </h3>
                    {(ed.period || ed.venue) && (
                  ...
                  <div className="absolute top-0 right-0 -mt-10 -mr-10 text-9xl font-bold text-gray-100 dark:text-gray-700/50 select-none">{ed.year}</div>
              ...
              {editions.length === 0 && <p className="text-sm text-text-muted">開催年度の情報がありません。各回の結果を収録し次第、ここに年度を並べます。</p>}
            </div>
          </section>
        </div>
      </PageLayout>
...
export const getStaticProps = async () => {
  ...
  const editions: EditionCard[] = years.map((year) => {
    ...
    return {
      ...
      period: meta?.period ? `${meta.period.start.replace(/-/g, '/')}〜${meta.period.end.replace(/-/g, '/')}` : null,
