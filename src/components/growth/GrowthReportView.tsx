  ...
  return (
    <div className="grid gap-3 border-t border-gray-100 py-3 first:border-t-0 dark:border-gray-700 md:grid-cols-[1.4fr_1fr_1fr_0.8fr]">
      <div>
        ...
        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{metric.summary}</p>
      </div>
      ...
      <div className="flex items-start justify-between gap-2 md:block">
        ...
        <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">{confidenceLabel[metric.confidence]}</p>
      </div>
    </div>
  );
};
...
export const PracticeThemes = ({ themes }: { themes: GrowthReport['practiceThemes'] }) => (
  <section className="rounded-lg border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
    <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">練習テーマ</h2>
    {themes.length > 0 ? (
      <div className="mt-4 grid gap-3 md:grid-cols-3">
        {themes.map((theme) => (
          <div key={theme.id} className="rounded border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/60">
            <p className="text-xs font-medium text-gray-500 dark:text-gray-400">テーマ {theme.priority}</p>
            <h3 className="mt-1 font-semibold text-gray-900 dark:text-gray-100">{theme.title}</h3>
            <p className="mt-2 text-sm leading-6 text-gray-600 dark:text-gray-300">{theme.description}</p>
          </div>
        ))}
      </div>
    ) : (
      <p className="mt-3 text-sm text-gray-600 dark:text-gray-300">今回は大きく注意する項目はありません。次の試合でも同じ観点を見てみましょう。</p>
    )}
  </section>
);
