...
/*
  normalize-core.js
  Browser + Node 両対応の正規化モジュール。
  ロジックは scripts/normalize-to-participants-entries.cjs から抽出したもの。
  - Node: const { normalizeResults, serializeOutput } = require('.../normalize-core.js')
  - Browser: <script src="normalize-core.js"></script> → window.NormalizeCore
*/
(function (root, factory) {
  ...
  function normalizeResults(data, entriesMetaInput) {
    ...
    if (
      Array.isArray(data.roundRobinMatches) &&
      data.roundRobinMatches.length
    ) {
    ...
    for (const p of participantsMap.values()) {
      if (p && p.prefecture)
        p.prefecture = normalizePrefectureName(p.prefecture);
    }
    ...
    const oldToNewId = new Map();
    {
      ...
      for (const p of participantsMap.values()) {
        ...
        if (!newMap.has(newId)) {
          ...
          if (!exist.prefecture && p.prefecture)
            exist.prefecture = p.prefecture;
    ...
    if (
      ...
      for (const row of data.roundRobinMatches) {
        ...
        if (!entriesMap.has(key)) {
          ...
          const e = entriesMap.get(key);
          if (
            (!Array.isArray(e.playerIds) || !e.playerIds.length) &&
            playerIds.length
          ) {
            e.playerIds = playerIds;
          }
    ...
    for (const [gk, rows] of groups.entries()) {
      ...
      const scoresObj = {};
      const keyA =
        entryA != null ? String(entryA) : `P:${(pairA || []).join('|')}`;
      const keyB =
        entryB != null ? String(entryB) : `P:${(pairB || []).join('|')}`;
      if (scoreA != null) scoresObj[keyA] = scoreA;
      if (scoreB != null) scoresObj[keyB] = scoreB;
      const winnerEntryNo =
        scoreA != null && scoreB != null
    ...
    for (const row of rrRaw) {
      ...
      if (entryNo) {
        if (!rrPlayersByGroup.has(group))
          rrPlayersByGroup.set(group, new Set());
    ...
    for (const [k, perspectives] of rrMap.entries()) {
      ...
      if (entryA && entryB) {
        ...
        if (pa && pa.row.games) {
          scoreA = Number(
            (pa.row.games.won != null ? pa.row.games.won : pa.row.games.for) ||
              0,
          );
          scoreB = Number(
          ...
          scoreB = Number(
            (pb.row.games.won != null ? pb.row.games.won : pb.row.games.for) ||
              0,
          );
          scoreA = Number(
        ...
        if (pr && pr.games) {
          ...
          if (en) {
            ...
            scoreB = slost;
            winnerEntryNo =
              swon > slost ? Number(en) : swon < slost ? null : null;
          }
    ...
    for (const m of matchesTransformed) {
      ...
      for (const cand of candidates) {
        ...
        for (const en of candEntries) {
          ...
          // intersection
          const intersect = enPlayerIds.some((id) =>
            winnerPlayerIds.includes(id),
          );
          if (intersect) {
    ...
    for (const m of matchesTransformed) {
      ...
      if (Array.isArray(m.entries)) {
        for (const en of m.entries)
          if (en != null) knockoutEntries.add(String(en));
      }
    }
  ...
  function serializeOutput(outObj) {
