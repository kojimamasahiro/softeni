...
def normalize_school_name(name):
    ...
    **「中学校」「中等教育学校」は落としてはいけない**（2026-08-13 修正）。
    落とすと `四天王寺中学校` → `四天王寺` となって同名の**高校**に一致し、
    中高一貫校の中学の成績が高校の学校ページに混ざる。実際に55校・242件が混入していた。
    これは ADR-013 で JS 側の名寄せに導入した「中高一貫校は世代で分ける」規約と同じ問題で、
    この Python パイプラインだけ対策が入っていなかった。

    安全性: 高校チーム一覧793件のうち名前に「中学」「中等教育」を含むのは
    `土佐中学校` `新宮中学校` `日高中学校` の3件だけで、これは高校地区大会への
    中学校名混入（チェックリスト C-6）という別の既知問題。いずれも
    `normalize_team_name` の `if team_name in team_map` で早期リターンするため、
    ここを変えても既存の高校マッチングは変わらない。
    """
    remove_words = [
        "高校",
        "高等学校",
