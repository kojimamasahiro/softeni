...
if (!data) {
  ...
  // 単調増加は**制度変更後だけ**に要求する（2026-09-13 に範囲を狭めた）。
  //
  // 制度変更前はクラブ数が1団体で固定されており（2018 中之条クラブ / 2019 日野クラブ /
  // 2021・2022 各1）、比率は 1/N で分母だけが動く。実際 2018→2019 は
  // 1/130=0.769% → 1/134=0.746% と下がるが、これは全中の出場団体数が増えただけで
  // 地域移行が後退したわけではない。2026-09-03 に2018・2019年度を投入した時点から
  // この判定は落ち続けていた。
  //
  // 主張したいトレンド（2023年度以降の 10.3% → 25.2% → 36.8% → 54.0%）はこの判定で守られ、
  // 制度変更の前後で跳ねたことは上の「前の最大 < 後の最小」が別途守っている。
  check(
