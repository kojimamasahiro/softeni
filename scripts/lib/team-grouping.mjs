 ...
 * （2026-09-06 修正）。
 *
 * **名前を出場大会のジャンルより優先する**（2026-09-12 修正）。
 *
 * なぜか: 2026-09-12 の抜き取り点検（機械だけで決めた統合49件から20件）で、
 * **誤統合が11件（55%・95%区間 34.2〜74.2%）**と出た。その多くが「クラブが中学校に統合された」型で、
 * 原因は `memGenre`（出場大会のジャンル）が名前より優先されていたこと。
 * 例: `仙北STC` は名前にSTCとあるのに、出場大会が中学の大会だけだったため段階が「中」と判定され、
 * `仙北中学校` 系と同じグループに入りうる状態だった。
 *
 * 人の判断（ユーザー、2026-09-12）:
 *   - 小学・中学・高校は**同時に発生しない**ので、段階が違えば必ず別チーム
 *   - 「学校」と付く名前と、クラブ・STC などのクラブチームは別
 *
 * 名前は「その団体が何と名乗っているか」そのものなので、出場大会からの推定より強い証拠になる。
 * ジャンルは名前から段階が読み取れないとき（素の地名など）の補完に格下げした。
 */
export function defaultGroups(members, context) {
  const keys = members.map((m, i) => {
    const stage = level(m.name) ?? memGenre(m, context);
