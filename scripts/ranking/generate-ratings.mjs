...
// 敗者側（格上）が「その時点の established 上位◯以内」であることを要求する追加条件
// （単なるレート差だけでなく、破った相手がトップ層である保証。2026-07-11 ユーザー要望）。
const UPSET_MIN_LOSER_TOP_PERCENTILE = rating.upset?.minLoserTopPercentile ?? 0.05;
// 上位◯%の境界を意味のある値にできる最低母数。これ未満の時期は upset を出さない。
const UPSET_MIN_ESTABLISHED_POOL = 50;
...
// 「established 上位◯%」の境界レート。大会（tid×year）が切り替わるたびに再計算する
// （大会開始時点の分布 = 先読みなし。試合ごとの再計算は不要な精度なので大会単位で足りる）。
let currentTournamentKey = null;
let loserRatingFloor = Infinity; // 母数不足時は Infinity = upset を出さない
const refreshLoserFloor = () => {
  const est = [];
  for (const [p, n] of N) if (n >= PROVISIONAL_MATCHES) est.push(R.get(p) ?? INITIAL);
  if (est.length < UPSET_MIN_ESTABLISHED_POOL) {
    loserRatingFloor = Infinity;
    return;
  }
  est.sort((a, b) => b - a);
  const idx = Math.max(0, Math.min(est.length - 1, Math.floor(est.length * UPSET_MIN_LOSER_TOP_PERCENTILE)));
  loserRatingFloor = est[idx];
};
...
for (const m of matches) {
  ...
  // upset 判定（更新前 = 事前レート）
  const tournamentKey = `${m.tid}\t${m.year}`;
  if (tournamentKey !== currentTournamentKey) {
    currentTournamentKey = tournamentKey;
    refreshLoserFloor();
  }
  const winnerProb = m.winner === 0 ? ea : 1 - ea;
  const loserSideRating = m.winner === 0 ? rb : ra;
  const established = [...m.sideA, ...m.sideB].every((p) => getN(p) >= PROVISIONAL_MATCHES);
  if (winnerProb <= UPSET_MAX_PROB && (!UPSET_REQUIRE_ESTABLISHED || established) && loserSideRating >= loserRatingFloor) {
...
const upsetsOut = {
  ...
  params: {
    ...
    provisionalMatches: PROVISIONAL_MATCHES,
    minLoserTopPercentile: UPSET_MIN_LOSER_TOP_PERCENTILE,
    minEstablishedPool: UPSET_MIN_ESTABLISHED_POOL,
    note: 'expectedWinProb<=0.10 は特大（2025年実績で年14件程度）。0.15は候補プール（同33件程度）。敗者側は当時のestablished上位以内（minLoserTopPercentile）に限定',
  },
  count: upsets.length,
  events: upsets,
};
