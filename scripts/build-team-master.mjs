...
try {
  const at = JSON.parse(fs.readFileSync(path.join(ROOT, 'data', 'tournaments', 'team-name-aliases.json'), 'utf8'));
  for (const e of at.teamAliases || []) for (const a of e.aliases || []) aliasRaw.set(norm(a), e.canonical);
} catch {}
const walk = (d) =>
...
for (const f of walk(DET)) {
  ...
  for (const p of d.participants || []) {
    ...
    if (!raw) continue;
    const n = norm(raw);
    const canon = aliasRaw.get(n);
    const key = canon ? norm(canon) : n;
    inc(rawC, key, raw);
...
for (const key of [...tot.keys()].sort((a, b) => tot.get(b) - tot.get(a) || (a < b ? -1 : a > b ? 1 : 0))) {
  // count降順→名前で決定的タイブレーク（id安定化）
  // 表示名は最頻出の生表記だが、全角ASCIIは半角へ寄せる（ＹＫＫ→YKK, Ｊ－Ｋｉｄｓ→J-Kids）。
  // 半角化前の生表記は aliases 側に残るので、元の表記は失われない。
  const rc = rawC.get(key);
  const name = toHalfWidthAscii(top(rc)[0]);
  const aliases = [...rc.keys()].filter((r) => r !== name).sort();
  const total = tot.get(key);
  const pc = prefC.get(key);
  let prefecture = null,
    others = [];
  if (pc) {
    const real = [...pc.values()].reduce((a, b) => a + b, 0);
    const [mp, mc] = top(pc);
    const cov = real / total,
      share = mc / real;
  ...
  context[id] = {
    players: topN(playerC.get(key) || new Map(), 8),
...
console.log(
  ...
  teams.length,
  '/ prefecture=null:',
  teams.filter((t) => !t.prefecture).length,
  '/ aliases付き:',
  teams.filter((t) => t.aliases).length,
  '/ 同名別校疑い:',
  review.length,
);
console.log('上位5:');
for (const t of teams.slice(0, 5)) console.log('  ', JSON.stringify(t));
