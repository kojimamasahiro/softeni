...
#
# **本戦の前に「予選」を1試合だけ置く形式も対象外**（2026-09-06 追記）。
# 予選の敗者は本戦のドローに席を持たないので `entries[].type` は `preliminary` になるが、
# `entry_types` は枠数が2の冪にならず assert で、`link_matches` は "予選" が
# ROUND_ORDER に無く ValueError で、それぞれ**黙って壊れずに落ちる**。
# 実例は zennihon-singles/2017 男子（257名→予選1試合→本戦256枠）。
# 対応が要るようになったら lib/bracketLayout.ts のコメントを参照すること。
